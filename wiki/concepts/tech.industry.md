---
type: concept
title: ICT Industry
slug: tech.industry
places: [MRT, XGL, XAF, ZAF, NGA, MAR, KEN, EGY, GHA, SDN, CIV, SEN]
entities: [[david-eaves], [gocab], [yango], [moove], [camp], [the-govlab], [cassava-technologies], [mtn-group], [sita], [mosip], [amazon-web-services], [afcfta], [papss], [african-development-bank], [nafa], [sidi-ould-tah], [diode-network], [axiz], [equinix], [kasi-cloud], [orange], [amazon-leo], [spacecoin], [mtn-nigeria], [starlink], [airtel-africa], [vodacom], [axian-telecom], [econet-wireless], [ethio-telecom], [microsoft], [gsma], [research-ict-africa], [skydd], [nigerian-exchange], [bamboo], [cowrywise], [meristem-securities], [afrinvest], [aicha-niang], [truecaller]]
lens: [sovereignty, colonialism]
status: active
last_reviewed: 2026-07-20
sources: [[2025-12-15 The Path to a Sovereign Tech Stack is Via a Commodified Tech Stack  TechPolicy.Press], [2026-06-02 Eaves commodification], [2026-06-09 The Triad of Tech Sovereignty and Europe's Package Dependency, Openness, and Agency], [2026-06-23 Africa's Data Centre Boom Where The New Facilities Are Being Built], [2026-07-07 South Africa's state IT agency is slowing government tech rollout], [2026-07-03 Microsoft Launches Frontier Company with $2.5 Billion AI Transformation Investment], [2026-06-29 How AI Keeps Europe Hooked on US Cloud], [2026-06-11 The Case Against the EU's Tech Sovereignty Package], [2026-06-03 How the EU's Tech Sovereignty Package Finally Puts Open Source to the Test], [2026-06-10 MTN Group goes all-in on platforms and AI], [2026-04-09 Ralph Mupita on MTN's New Frontier Connectivity, Content, and African AI], [2026-05-01 Understanding-MOSIP-Biometric-Update-report], [2026-07-09 Nigeria Takes AfCFTA Council Chair as Digital Forum Charts Roadmap for Implementation], [2026-02-26 The AfDB's push for African financial sovereignty, and a tech investment rebound], [2026-04-28 Axiz Welcomes Equinix Investment, Citing Boost to Africa's Digital Infrastructure], [2026-07-11 AWS in Africa], [2017-04-18 Publications], [2026-01-15 Amazon Leo secures landing permit in Nigeria], [2026-01-13 Spacecoin strikes major deals to launch satellite internet], [2026-01-23 MTN Nigeria fibre network takes a hit], [2025-12-04 Is it a bird Is it plane No, Jim, it's an African mobile operator, but not as we know it 9], [2026-02-16 Waking the dead - Slow road to extinction for Africa's former telco incumbents 11], [2026-04-16 The Long View - What ICT4D did for Africa and why that deep past matters for today], [2026-02-23 111 - Winning But Losing - The Payments End Game], [2026-06-05 Microsoft Build 2026 What Caught My Attention and What It Means for Africa], [2026-07-17-skydd-insure-fintech-leadership], [2026-07-18-nigeria-retail-investing-boom], [2026-07-18-gocab-yango-abidjan-evs], [2026-07-17-senegal-yango-expands-into-corporate-mobility], [2026-07-17-truecaller-q2-sales-decline-recovery-signals]]
# Source-linkage: the MOSIP report details the vendor ecosystem / Marketplace as the market
# structure that makes open-source ID switchable (anti-lock-in). Prose enrichment pending.
---

# ICT Industry (`tech.industry`)

Structure and power dynamics of the technology industry — here, the cloud oligopoly and
Africa's emerging data-centre operators.

## Key material

- **Truecaller — a caller-ID platform's numbers, and why they are logged here (2026-07-17).** Q2
  2026 (quarter to June): net sales **SEK 393.2m**, down **21%** y/y (−14% constant currency); advertising
  revenue **−34%**; subscriptions **+41%** and Truecaller for Business **+8%** (constant currencies), taking
  recurring revenue to **49% of net sales** (from 33%). EBITDA ex-incentives **−50%** to SEK 105.8m (26.9%
  margin, from 42.6%); profit after tax **SEK 51.7m** (from SEK 118m). Average monthly active users on
  Android/non-iOS reached **471m** (from 426.6m), +8m in the quarter. CEO **Rishit Jhunjhunwala** cited a
  demand partner lifting restrictions, and the launch of **travel eSIMs** — its first move beyond caller ID
  and spam protection. **Caveat on scope: the report contains no African segment data**, so these figures
  are recorded as the trajectory of a standing entity whose consumer phone-number database is widely used
  across African markets — **not** as evidence about any African market. Per *cite, don't absorb*, none of
  these figures should be promoted to a place page. Source:
  [[2026-07-17-truecaller-q2-sales-decline-recovery-signals]].
- **The infrastructure turn: consumer fintechs repositioning as rails ([[NGA]], 2026-07).** Two
  Nigerian items a day apart show the same industry move — a company that sold to consumers deciding
  its product is the plumbing. **[[skydd]]** announced it is **opening its protection infrastructure to
  banks, fintechs, employers, merchants and developers** for embedded insurance, alongside its own
  consumer platform (protection, savings, wallets, cards); company figures put it at **>20,000 active
  customers with positive EBITDA and cash flows** (2026-07-17). *That item is a TechCabal Partner /
  Press Room sponsored placement — a first-hand company announcement, not independent reporting.*
  Meanwhile the retail-brokerage layer inverted: **[[bamboo]]** says it **overtook CardinalStone as the
  [[nigerian-exchange|NGX]]'s largest broker by weighted market share in April 2026** (11.13%, 542,582
  trades in the month), two years after adding Nigerian equities — an app-first entrant displacing an
  incumbent brokerage at the top of a national exchange league table, with incumbents
  (**[[meristem-securities]]**'s Meritrade, **[[afrinvest]]**'s PlutusNeo and Afrinvestor 2.0, Stanbic
  IBTC) digitising in response and the exchange launching **NGX Invest in 2024**. The industry-structure
  reading: distribution is moving to whoever owns the phone surface, and the firms that got there first
  are now selling access to it wholesale. See [[nigeria--dpi-pay]], [[dpi.pay]], [[include.access]].
  Sources: [[2026-07-17-skydd-insure-fintech-leadership]], [[2026-07-18-nigeria-retail-investing-boom]].
- **Platform power and "data colonialism" (2026 H1).** CIGI argues Africa is positioned to lead on
  regulating US tech giants that dominate its markets (Google ~90% search, Meta ~85% social) with little
  local reinvestment; Foresight Africa and CIGI both invoke the "raw data mine"/"data colonialism"
  framing, and data-centre localisation (Nigeria's demand that Google/Microsoft/Amazon host locally) is a
  live sovereignty battleground. Sources:
  [[2026-02-01 Why Africa Should Lead on Governing Global Tech Giants CIGI PB226]],
  [[2026-02-18 Foresight Africa 2026]],
  [[2026-03-01 Tech Diplomacy and Avenues for Digital Development in Africa CIGI PB232]]. See [[gov.regional]], [[infra.store]].

- **Concentration, quantified ("Parting Clouds," Eaves & Bester / [[camp]]).** Three
  hyperscalers hold ~**66%** of global public cloud (85% in Canada); costs of
  concentration include lock-in, opaque pricing, gatekeeping of adjacent markets,
  innovation capture, outage risk, and geopolitical coercion. The evidenced version of
  the commodification argument. Source: [[2026-06-02 Eaves commodification]].
- **The African operator layer (2026-06).** A domestic industry is forming —
  [[cassava-technologies]] (ADC), [[mtn-group]], [[airtel-africa]], IXAfrica, Rack Centre,
  Teraco — though **>half** of continental capacity is still in South Africa. MTN's
  naira-billed local cloud is a competitive challenge to hyperscaler pricing. Source:
  [[2026-06-23 Africa's Data Centre Boom Where The New Facilities Are Being Built]].
- **Ownership vs agency (Triad).** [[the-govlab]] reframes the industry question as
  distribution of power, not ownership. Source: [[2026-06-09 The Triad of Tech Sovereignty and Europe's Package Dependency, Openness, and Agency]].
- **Public-sector delivery capacity ([[sita]]).** The industry story has a state-capacity
  flip side: South Africa's central ICT agency cannot reliably procure/deliver. Source:
  [[2026-07-07 South Africa's state IT agency is slowing government tech rollout]].

- **Global (2025, Eaves).** A handful of US hyperscalers — **AWS, Microsoft Azure,
  Google Cloud** — control the digital substrate. The **"original sin of cloud"**: when
  Azure launched (2008) with storage incompatible with AWS **S3** (2006), lock-in
  became a feature, not a bug. Chokepoints are **IaaS** and **PaaS**; migration is
  costly (reconfigure data, rewrite code, retrain staff), producing vendor and career
  lock-in. Eaves warns the reflexive fix — backing a domestic telco/"national champion"
  — merely swaps one lock-in for another. See [[geopol.usa]], [[gov.standards]].

- **AfCFTA as the demand thesis (2026-07).** At the 2nd AfCFTA Digital Trade Forum (Lagos,
  Jul 2026) Africa's digital economy was valued at **~$180bn now, rising to ~$712bn by 2050**;
  the Forum's Eight Calls to Action foreground digital infrastructure, interoperable payments
  ([[papss]]) and cross-border data — the continental market signal the industry is building
  toward. See [[afcfta]], [[gov.regional]], [[dpi.pay]]. Source:
  [[2026-07-09 Nigeria Takes AfCFTA Council Chair as Digital Forum Charts Roadmap for Implementation]].
- **Investment rebound, concentrated (2025 data).** Partech: African tech VC **$4.1bn in
  2025 (+25%)**, the strongest since 2022 — but debt reached 41% of capital, the **Big Four
  (KEN/ZAF/EGY/NGA) took 72%**, domestic investors rose to 45%, and the seed pipeline keeps
  contracting. [[african-development-bank]]'s **[[nafa]]** ([[sidi-ould-tah]]) supplies the
  "financial sovereignty" macro-frame. See [[finance.new]]. Source:
  [[2026-02-26 The AfDB's push for African financial sovereignty, and a tech investment rebound]].
  **Read "rebound" with care: it is a Partech-scoped, debt-carried figure.** On *equity alone* the two
  trackers point opposite ways for CY2025 — Partech **+8% to US$2.41bn**, AVCA **−21% to US$2.1bn** — and
  the divergence sits in their **2024 denominators** (Partech US$2.2bn; AVCA ≈US$2.7bn, a base AVCA does
  not print), not in a revision by either. Resolved 2026-07-20; see [[finance.new]] for the full
  reconciliation. Sources: [[2026-01-22-partech-africa-tech-vc-report-2025]],
  [[2026-02-10-avca-venture-capital-in-africa-report-2025]].
- **Operator & channel layer thickening (2026-04).** Hyperscaler capacity —
  [[amazon-web-services]]' Cape Town region, Microsoft — is increasingly intermediated by a
  domestic channel: **[[axiz]]**, [[equinix]]'s SA distribution partner, packaging
  AI/hybrid-cloud/data-sovereignty demand for local partners, alongside emerging local clouds
  (e.g. [[kasi-cloud]]) and pan-African carriers such as [[orange]]. SA still anchors the
  market and acts as the continental gateway. See [[infra.store]], [[ZAF]]. Sources:
  [[2026-04-28 Axiz Welcomes Equinix Investment, Citing Boost to Africa's Digital Infrastructure]],
  [[2026-07-11 AWS in Africa]].
- **MNO state-of-market — from telcos to holding companies (Russell Southwood, 2025-12–2026-06).**
  Southwood's recurring analysis reframes the African operator layer: as **data revenue
  overtakes voice** (by end-FY2025 for Airtel; Vodacom calls prepaid voice a "managed
  decline"), the four "power players" — **[[mtn-group]], [[airtel-africa]], [[orange]],
  [[vodacom]]** — are becoming **data/holding companies**, spinning out towers, data
  centres, wholesale ([[mtn-group]]'s Bayobab brand) and mobile money as arms-length
  businesses, adopting network-sharing/JVs to bear fibre and AI CAPEX, and struggling in
  "digital life" verticals against the hyperscalers. Below them sit a "second division"
  ([[axian-telecom]], [[econet-wireless]], Africell, Globacom), the state incumbents
  ([[ethio-telecom]]) and — his sharpest case — the **former fixed-line incumbents as
  "stranded assets"** (Telkom Kenya, Sotel Tchad, Gamtel, Camtel, SCPT), undone by absent
  vision, capital, skills and patronage/corruption, with only Camtel profitable and only via
  protected wholesale/landing-station monopolies. His ICT4D long view roots this in the
  2000s liberalisation wave that mobile transformed but whose network effects re-created
  monopolies. See [[infra.connect]], [[tech.ai]], [[infra.store]]. Sources:
  [[2025-12-04 Is it a bird Is it plane No, Jim, it's an African mobile operator, but not as we know it 9]],
  [[2026-02-16 Waking the dead - Slow road to extinction for Africa's former telco incumbents 11]],
  [[2026-04-16 The Long View - What ICT4D did for Africa and why that deep past matters for today]].
- **Payments margins collapse to a utility (Samora Kariuki, 2026-02-23).** A parallel
  structural read of the *payments* industry: as low-cost central-bank instant-payment rails
  become utilities (Brazil Pix, India UPI, Africa's IPS at **$1.98T** in 2024), processing
  margins collapse and durable value migrates to credit, data and compliance layers — the
  same "infrastructure commoditises, value moves up the stack" dynamic. See [[dpi.pay]],
  [[finance.new]]. Source: [[2026-02-23 111 - Winning But Losing - The Payments End Game]].
- **Vendor vertical-integration at the top (Anjola Adebowale, 2026-06-05).** Adebowale reads
  Microsoft Build 2026 as **[[microsoft]]** assembling a full-stack, vertically-owned AI
  platform (own MAI models, tuning, agents, quantum, hardware) — the industry's concentration
  logic extended from cloud into models and silicon, with Africa's risk being engagement on
  others' terms. See [[geopol.usa]], [[infra.store]]. Source:
  [[2026-06-05 Microsoft Build 2026 What Caught My Attention and What It Means for Africa]].
- **Research base (resource).** The **[[diode-network]]** (Univ. Manchester GDI; Heeks et al.)
  curated bibliography on the digital economy and digital labour in the Global South is the
  standing literature shelf for the industry's political economy — gig/platform labour,
  digital enterprise in Africa, latecomer industrial policy. Filed `needs-review`; entries
  promoted to `publication` only on citation. See [[capacity.research]]. Source:
  [[2017-04-18 Publications]].

- **The satellite-broadband entrant field widens (2026-01).** The LEO industry in Africa is
  diversifying beyond [[starlink]]: [[amazon-leo]] (formerly Project Kuiper) won a 7-year Nigeria
  landing permit, and blockchain-integrated operator [[spacecoin]] secured a Kenyan transmission
  licence (continuing under an existing NGA licence) — two more players in the satellite-industry
  layer. On the terrestrial side, [[mtn-nigeria]]'s 2025 wrap paired a return to profit
  (PAT ~$525m, 9M to Sep 2025) with **9,218 fibre-cut incidents**, underscoring the operating
  risk in the West-African market. See [[infra.connect]], [[data.satellite]], [[NGA]], [[KEN]].
  Sources: [[2026-01-15 Amazon Leo secures landing permit in Nigeria]],
  [[2026-01-13 Spacecoin strikes major deals to launch satellite internet]],
  [[2026-01-23 MTN Nigeria fibre network takes a hit]].

- **Platform economics: from ride-hailing to asset finance ([[CIV]], 2026-07-18).** The ride-hailing
  layer of the African ICT industry is becoming a **credit-and-asset business**. **[[gocab]]** — owner of
  **[[yango]]'s** largest Côte d'Ivoire fleet, **$45m raised Feb 2026** — put **100 electric cars** into
  Abidjan under **drive-to-own** terms: drivers repay from platform earnings over **three years**, then
  take title, with **300+ drivers** reaching ownership **from 2027**. The platform holds the earnings
  data, the repayment stream and the asset at once, which makes ride-hailing a **consumer-lending channel
  for drivers outside formal banking** — the structure behind [[moove]]'s **$2bn valuation (2026-07)**.
  Note also the **market-entry thesis**: Yango Africa CEO Adeniyi Adebayo said (May 2026) the group would
  put **at least $150m into African expansion in 2026** across **10 new markets**, deliberately avoiding
  Nigeria, Egypt, South Africa and Kenya for **secondary cities** in West and Central Africa, on the view
  that **cities, not countries, are the unit of analysis** — Bouaké (launched 2022, flat three years, now
  a top performer) the cited proof. See [[dpi.pay]], [[infra.energy]], [[civ--tech-innovate]]. Source:
  [[2026-07-18-gocab-yango-abidjan-evs]].
- **Tower unbundling reaches [[ZWE]] Zimbabwe (2025-12 → 2026-03).** Econet Wireless Zimbabwe consolidated
  its real estate and passive telecoms infrastructure out of EWPL and other wholly owned SPVs into
  **Distributed Power Africa (Private) Limited, trading as [[econet-infraco|Econet InfraCo]]**, as a
  going-concern transfer under a Scheme of Reconstruction Agreement completed **2025-12-24**; InfraCo listed
  on the USD-denominated **VFEX on 2026-03-31** while EWZL delisted from the ZSE after 27 years. The stated
  driver is a valuation gap — EWZL trading at a discount to African peers at **6–8× EV/EBITDA** who had
  already separated their towers. Final ownership **70.000% EWZL / 25.215% Econet shareholders / 4.785%
  exiting shareholders**. Sources:
  [[2025-12-24-econet-notice-completion-infrastructure-restructuring]],
  [[2026-03-26-econet-infraco-supplementary-pre-listing-statement]].
- **Before quoting any African VC total, check which universe it counts.** Partech CY2025:
  **US$4.1bn / 570 deals** equity+debt, of which **equity US$2.41bn (+8%)**. AVCA's comparable
  equity+debt total is **US$3.9bn / 506 deals**, its equity-only line **US$2.1bn (−21%)**. Disrupt Africa
  publishes a **self-declared minimum floor by funded venture**, **US$1.6367bn / 178 startups**. These are
  four different measurements, not four estimates of one quantity — see [[finance.new]].

## Places

[[XGL]] · [[XAF]] · [[ZAF]] · [[NGA]] · [[MAR]] · [[KEN]] · [[EGY]] · [[GHA]] · [[CIV]]

## Related

[[geopol.usa]] · [[infra.store]] · [[gov.standards]] · [[gov.policy]] · [[gov.legislate]]

## Sources

- [[2026-07-17-skydd-insure-fintech-leadership]]
- [[2026-07-18-nigeria-retail-investing-boom]]
- [[2026-07-18-gocab-yango-abidjan-evs]]
- [[2025-12-15 The Path to a Sovereign Tech Stack is Via a Commodified Tech Stack  TechPolicy.Press]]
- [[2026-06-02 Eaves commodification]]
- [[2026-06-09 The Triad of Tech Sovereignty and Europe's Package Dependency, Openness, and Agency]]
- [[2026-06-23 Africa's Data Centre Boom Where The New Facilities Are Being Built]]
- [[2026-07-07 South Africa's state IT agency is slowing government tech rollout]]
- [[2026-07-03 Microsoft Launches Frontier Company with $2.5 Billion AI Transformation Investment]]
- [[2026-06-29 How AI Keeps Europe Hooked on US Cloud]]
- [[2026-06-11 The Case Against the EU's Tech Sovereignty Package]]
- [[2026-06-03 How the EU's Tech Sovereignty Package Finally Puts Open Source to the Test]]
- [[2026-06-10 MTN Group goes all-in on platforms and AI]]
- [[2026-04-09 Ralph Mupita on MTN's New Frontier Connectivity, Content, and African AI]]
- [[2026-07-09 Nigeria Takes AfCFTA Council Chair as Digital Forum Charts Roadmap for Implementation]]
- [[2026-02-26 The AfDB's push for African financial sovereignty, and a tech investment rebound]]
- [[2026-04-28 Axiz Welcomes Equinix Investment, Citing Boost to Africa's Digital Infrastructure]]
- [[2026-07-11 AWS in Africa]]
- [[2017-04-18 Publications]]
- [[2026-01-15 Amazon Leo secures landing permit in Nigeria]]
- [[2026-01-13 Spacecoin strikes major deals to launch satellite internet]]
- [[2026-01-23 MTN Nigeria fibre network takes a hit]]
- [[2025-12-04 Is it a bird Is it plane No, Jim, it's an African mobile operator, but not as we know it 9]]
- [[2026-02-16 Waking the dead - Slow road to extinction for Africa's former telco incumbents 11]]
- [[2026-04-16 The Long View - What ICT4D did for Africa and why that deep past matters for today]]
- [[2026-02-23 111 - Winning But Losing - The Payments End Game]]
- [[2026-06-05 Microsoft Build 2026 What Caught My Attention and What It Means for Africa]]
