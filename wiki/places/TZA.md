---
type: place
title: Tanzania
code: TZA
parent: XEA
place_kind: country
topics: [dpi.id, dpi.registry, dpi.pay, dpi.exchange, dpi.govtech, dpi.mis, gov.protect, gov.legislate, gov.standards, gov.policy, gov.regional, infra.store, infra.connect, infra.cybersec, include.access, include.divides, digital.rural, digital.localgov, data.statistics, data.satellite, tech.ai, tech.innovate, tech.industry, capacity.training, capacity.literacy, finance.new]
status: active
last_reviewed: 2026-07-23
---

# Tanzania (TZA)

East African ([[XEA]]) state that has largely **built its own digital public infrastructure rather
than bought it**. The **[[bank-of-tanzania]]** built **[[tips-tanzania]]**, the national real-time
retail payment switch, **in-house from 2018**; it carried **over 453.7m interoperable retail
transactions worth over TZS 29.9 trillion in 2024**. **[[nida-tanzania]]** runs a national ID at
**81.9% of the adult population (April 2025)**. **[[napa-tanzania]]** has been registering physical
addresses **since 2010**, to a **[[upu-s42]]** certification the [[papu]] Secretary General called a
continental benchmark (2026-02). The **[[tanzania-pdpa]]** has been in force **since 1 May 2023**
with a supervisory commission and a registration deadline that **expired 30 April 2025**.

The tension is **capable delivery alongside real definitional softness in the law, and an unsolved
last mile**. The PDPA leaves **"security breach" and "without undue delay" undefined**, offers **no
adequacy mechanism** for cross-border transfers, imposes **no duty to notify data subjects**, and
narrows Zanzibar to Union Matters — a statute that arrived fast and is being given meaning by
practitioners rather than by its text. Meanwhile **300,000+ processed ID cards sat uncollected
(2026-01)**, an ID application still needs a physical **Local Chairman's stamp** before NIDA will see
it, and the **infant-registration pilot ran roughly six months ahead of its enabling Bill**. Tanzania
is not short of build capacity; it is short of definition, and of delivery at the edge.

## Extracted intersections

The identity, data-protection and addressing material is compiled in full on its own pages; this hub
carries the framing and the hub-level chronology only.

- **[[tanzania--dpi-id]]** — [[jamii-namba]] / [[nida-tanzania]]: coverage corroborated by two
  independent routes at ~81–82% of adults (2025); infant registration and the mother's-ID linkage;
  the pilot-ahead-of-Bill finding; 300,000+ uncollected cards; Phase II iris/facial; the Local
  Chairman's stamp; the [[nao-tanzania]] performance audit.
- **[[tanzania--gov-protect]]** — the [[tanzania-pdpa]] and [[pdpc-tanzania]]: scope, extraterritorial
  reach, the Zanzibar Union-Matters limit, cross-border transfers without an adequacy mechanism,
  breach notification unconditioned on harm but textually undefined, penalties, the BOT
  consumer-protection cross-reference, and the CDPO programme. Now with the **enforcement arc** (awareness
  2025 → **inspections from 9 Apr 2026** → 7-day ultimatum → first National Privacy Conference), the
  forthcoming **National Data Management Framework** (targeted Jan 2026), and a critical **IGP/TMC**
  counter-read that data-localisation/surveillance is a growth *bottleneck*, not just a safeguard.
- **[[tanzania--dpi-registry]]** — [[napa-tanzania]] addressing since 2010; the 85% rate (2024-07) and
  the 12.9m address count (2026-02) as non-comparable metrics; [[upu-s42]] certification as a
  continental benchmark; 29 systems interconnected.

## Recent developments

*Selective — hub-level items. Identity, data-protection and addressing chronology lives in the
intersections above.*

- **2026-07-23 (reported; visit date not stated)** — **TTCL runs hands-on NICTBB training for DIT
  students.** [[ttcl]] hosted Dar es Salaam Institute of Technology students at its headquarters for
  practical training on the **[[nictbb]]** — technologies, infrastructure and operational systems —
  pitched as bridging classroom and industry for the national backbone's future workforce. No intake
  number or programme cadence is published. Source: [[2026-07-23-ttcl-dit-nictbb-training]].
  [[capacity.training]] · [[infra.connect]]
- **2026-07-15→20** — **Three bilateral digital-cooperation tracks opened in one week.** At WSIS
  (Geneva) minister Kairuki agreed cooperation frameworks with **Brazil** (AI strategy, Gov.br, PIX
  experience; MoU proposed; investment invited into data centres) and **Poland** (cybersecurity,
  digital government, AI research; MoU proposed); follow-up talks in Dar es Salaam with **Russia's
  Rosmolodyozh** ([[ict-commission-tanzania|ICTC]]) drafted youth/tech-skills agreements. All
  intent-stage — no signed instrument, amount or timetable held. Sources:
  [[2026-07-17-tza-brazil-poland-wsis-mous]], [[2026-07-20-tza-russia-tech-frontier]].
- **2026-07-10** — **First EAC cross-border instant-payment corridor.** A technical meeting in
  Zanzibar on a proof of concept interlinking [[tips-tanzania]] with Rwanda's
  **[[rswitch-rwanda]]** — the first corridor under the
  **[[eac-cross-border-payment-masterplan]]** ([[eac]]), under "variable geometry" (only RWA+TZA so
  far), initially P2P, **no launch date**. Partners: central banks of RWA/TZA, EAC Secretariat,
  [[world-bank]], [[giz]]; chair Nestory Nicholas Maro (BoT). Source:
  [[2026-07-10 New Rwanda-Tanzania Payment Pilot could transform trade across East Africa]].
  [[dpi.pay]] · [[gov.regional]] · [[RWA]]
- **2026-05-21** — Tanzania grouped with Morocco, Djibouti, Ethiopia and Ghana as second-tier African
  data-centre markets projected to draw a combined **>$1.3bn by 2031** (Arizton). Source:
  [[2026-05-21 Africa Data Center Investment 2026 The $4.5B Race and the Power Problem]].
  [[infra.store]]
- **2026-04-21** — **Safiri** (Dar es Salaam; transport/logistics) joins
  [[google-for-startups-accelerator-africa]] Class 10. Source:
  [[2026-04-21 Meet the 15 Startups Joining the Google for Startups Accelerator Africa Class 10]].
  [[tech.innovate]]
- **2026-04-16** — Russell Southwood's ICT4D long view cites Tanzania (with Cameroon) as a coastal
  incumbent **gatekeeping international bandwidth** for landlocked neighbours in the SAT3/WACS era.
  Source:
  [[2026-04-16 The Long View - What ICT4D did for Africa and why that deep past matters for today]].
  [[tech.industry]] · [[infra.connect]]
- **2026-04-13** — **Helios Towers completes 133 UCSAF rural sites**, extending coverage to
  rural/underserved areas (~45% of its Tanzania sites are rural); marked in Dodoma with President
  Samia Suluhu Hassan and ICT Minister Angela Kairuki. Source:
  [[2026-04-13 Helios Towers completes 133 UCSAF sites to expand rural connectivity in Tanzania]].
  [[infra.connect]] · [[digital.rural]] · [[include.access]] · [[digital.localgov]]
- **2026-04-13** — At the 58th [[uneca]] session, Deputy Minister (President's Office – Planning and
  Investment) **Pius Stephen Chaya** argued DPI impact needs a public-private approach anchored in
  robust cybersecurity and data-protection law to safeguard sovereignty. Source:
  [[2026-04-13 Stakeholders urge shift in digital public infrastructure trajectory]].
  [[dpi.exchange]] · [[gov.regional]] · [[gov.policy]]
- **2026-04-01** — **Wingu Cloud Exchange debuted in Tanzania (Nov 2025).** [[wingu-africa]]'s
  private-cloud **WCX** platform — in-country data residency, local-currency pricing, hybrid with
  Azure/AWS — launched **first in Tanzania in November 2025**, before its Ethiopia ([[ETH]]) rollout,
  after a **$60m** raise. *A separate object from the $50m Mbezi data-centre phase below.* Source:
  [[2026-04-01 Wingu Africa debuts cloud exchange platform in Ethiopia]]. [[infra.store]] ·
  [[dpi.exchange]] · [[gov.protect]]
- **2026-03-31** — Tanzania is one of eight [[eac]] members weighing a regional **AI Policy Roadmap**
  to harmonise rules and keep AI's value in East Africa (4th EAC STI Conference, Kigali). Source:
  [[2026-03-31 EAC moves to create unified AI Sovereignty bloc]]. [[gov.regional]] · [[tech.ai]]
- **2026-03-05** — A [[research-ict-africa]] report on Tanzania's 2025 electoral cycle documents
  **state-led internet shutdowns**, media capture, digital divides and political disinformation, and
  argues global information-integrity interventions are largely ineffective in African contexts.
  Source: [[2026-03-05 Safeguarding Tanzania's information integrity]]. [[include.divides]] ·
  [[gov.policy]] *(no shutdown/disinformation slug — tagged to nearest)*
- **2026-01-01** *(fieldwork 2022; CHI '26)* — CMU-Africa study: biometric SIM verification blocks
  ID-borrowing, but ID-required mobile money still pushes users onto agent-registered SIMs. →
  [[tanzania--dpi-id]]. [[dpi.id]] · [[dpi.pay]]
- **2026-01-01** — [[starlink]] ([[spacex]]) signed [[airtel-africa]] (Direct-to-Cell, 14 markets)
  and [[vodacom]] (LEO backhaul + reseller, 25 countries); Starlink was live in 25 African markets
  **including Tanzania as of Nov 2025**. Deals: [[starlink-airtel-d2c-partnership]],
  [[starlink-vodacom-leo-partnership]]. Source:
  [[2026-01-01 Starlink Partners with Airtel & Vodacom From Rival to Ally]]. [[infra.connect]] ·
  [[data.satellite]]
- **2025-06-12/13** *(event, needs-review)* — **Fintech Festival Tanzania 2025** (Dar es Salaam),
  including a Bank of Tanzania fintech-sandbox case study. Marginal event profile pending review.
  Source: [[2025-01-14 Conference Sessions]]. [[dpi.pay]] · [[tech.innovate]]
- **2025-05-16** *(`date_source: proxy` — no date printed; the stamp is a CMS filename)* — **TIPS, a
  switch the central bank built itself.** [[tips-tanzania]] is a national real-time retail payment
  switch **built and operated in-house by the [[bank-of-tanzania]]**, from **2018**, with **[[fsdt]]**
  and **[[gates-foundation]]** support on **Level One Project** principles. **Over 453.7m
  interoperable retail transactions worth over TZS 29.9 trillion in 2024**; **63.2m active mobile
  money wallets at end-2024** ([[tcra]] Q4 2024). Lineage: a **2006 BoT Act amendment** gave BOT a
  mandate over non-banks; a **2007 "letter of no objection"** enabled mobile money. Source:
  [[2025-05-16 Building an Inclusive Payments Ecosystem in Tanzania through TIPS (BOT and FSDT)]].
  [[dpi.pay]] · [[dpi.exchange]] · [[include.access]]
- **2025-03-19** — **Wingu's $50m Tier 3 phase 2 at Mbezi.** [[wingu-africa]] launched **phase 2** of
  its Tanzania data centre — **$50 million**, **Tier 3**, **Mbezi Industrial Area**, Dar es Salaam —
  *"the most connected data centre in Tanzania, with interconnections to 13 operators"*, a **99.983%
  uptime** guarantee, adding **80 high-density racks for a total of 110**. CEO
  **[[anton-volianskyi]]**. **[[tspar]]** SG **Noah Maina** said extending the **internet exchange**
  to Wingu keeps **data traffic local**. *Distinct objects, easily confused: this **$50m data-centre
  phase (Mar 2025)** is not the **WCX platform (Nov 2025)**; the **$50m investment** is not Wingu's
  **$60m raise**; and **99.983% is Mbezi**, where the 99.982% on [[wingu-africa]] is the Djibouti TO7
  site.* Source: [[2025-03-19 Tanzania's digital shift gathers pace with $50 million data centre]].
  [[infra.store]] · [[infra.connect]] · [[sovereignty]]
- **2024-10-17** *(published 2024-10-18)* — **The NIDA number *is* the Jamii Namba.** PM
  **Kassim Majaliwa** closed the 8th ICT Congress (JNICC, Dar es Salaam) by stating the government
  had directed that **the NIDA number is the Jamii Namba** for delivering and receiving services, and
  ordered all public and private institution heads to make their systems interoperable so
  **[[jamii-x-change]]** completes as planned. ICT Minister **Jerry William Silaa** said the ministry
  was coordinating **Jamii Namba, Jamii X-Change and Jamii Portal**, plus **[[napa-tanzania|NaPA]]**
  (credited in the Hanang landslides and Kibiti floods; digital proof-of-address letters in pilot).
  Source: [[2024-10-18-hakikisheni-mnapata-namba-ya-nida-jamii-namba-majaliwa]]. → [[tanzania--dpi-id]],
  [[tanzania--dpi-govtech]]. [[dpi.id]] · [[dpi.exchange]] · [[dpi.registry]]
- **2024-08-27** — **Jamii X-Change under construction; the stack is three modules.** MICIT PS
  **Mohammed Khamis Abdulla**, at a Digital Tanzania Project review in Arusha, said
  [[micit-tanzania]] with [[ega-tanzania|e-GA]] was building **[[jamii-x-change]]** to execute
  President Samia's systems-interoperability directive, comprising **Jamii Namba, Jamii Kadi and Jamii
  Malipo** — the last an app *still to be built*, to link via the card to **[[tips-tanzania|TIPS]]**.
  *Jamii Malipo sits with MICIT/e-GA, not the [[bank-of-tanzania]].* Source:
  [[2024-08-27-mfumo-jamii-x-change-kuunganisha-mifumo-tehama]]. → [[tanzania--dpi-govtech]],
  [[tanzania--dpi-pay]]. [[dpi.exchange]] · [[dpi.pay]] · [[sovereignty]]
- **2024-03-08** *(published 2024-03-12)* — **Jamii Namba: design complete, end-2025 target (missed).**
  At the Conference of Financial Institutions, [[ega-tanzania|e-GA]] DG **Benedict Ndomba** and Dr
  **John Sausi** (Finance & Planning) said the design was finished — built by **[[rita-tanzania]],
  [[nida-tanzania]] and Zanzibar ID** — to identify Tanzanians *from birth* and foreigners *entry to
  exit*, concluding **end-2025**. Origin is President Samia's **2023-08-10** directive against
  fragmented identifiers, framed as a national-security problem. Source:
  [[2024-03-12-jamii-namba-development-work-started-chanzo]]. → [[tanzania--dpi-id]]. [[dpi.id]] ·
  [[sovereignty]]
- **2023-10-30** *(study)* — [[hmis-tanzania-pea]]: a **fragmented ecosystem of 18 systems**;
  [[dhis2]] (all district councils since 2013) carried **>90% of data flows**; barriers are staff
  capacity, finance and **closed-source lock-in**, with **long-relationship donors** (Basket Fund)
  most shaping investment ([[usaid]], [[world-bank]]). Source:
  [[2023-10-30 A political economy analysis of strengthening health information system in Tanzania]].
  [[dpi.mis]] · [[gov.standards]]
- **2021-09-13** *(study)* — [[rhis-recording-burden-5country]] finds **Tanzania the heaviest** of
  five: **48 registers** (34-country average) and **~13h per health worker per month** reporting.
  Source:
  [[2021-09-13 The burden of recording and reporting health data in primary health care facilities in five low- and lower-middle income countries]].
  [[dpi.mis]] · [[data.statistics]] · [[MOZ]] · [[NGA]]

## Financing

*Aggregate of tracked digital-transformation finance, compiled from the deal records in `raw/`. Totals are time-varying — **as of 2026-07-22**.*

**Non-state** — US$1.06bn committed across 35 deals (2015–2026). Top financiers: World Bank (US$150m), Economic Development Cooperation Fund (Korea Eximbank) (US$135m), Yas Fibre (Axian Telecom) (US$114m). Leading subjects: `dpi.pay`, `infra.connect`, `tech.innovate`.

Instrument mix: Grant 19, Equity 4, Concessional loan 3, Buyer's credit 1, Commercial loan 1, Financing package 1, Guarantee 1, Corporate investment 1, Credit facility 1, Public-private partnership 1. **1 record here still carries the capture's unverified `Concessional loan` label**, so the mix is indicative rather than established: the World Bank portal does not separate an IDA credit from an IDA grant.

**Domestic state** — *(pending that dataset)*.

Material deals: [[2021-05-27-wb-tza-001-digital-tanzania-project]], [[2022-01-01-axian-tza-tigo-acquisition-2022-axian-rostam-consortium-acquisition-of-tigo-tanzania]].

## Active topics

- [[dpi.id]] · [[gov.protect]] · [[dpi.registry]] · [[gov.legislate]] · [[gov.standards]] ·
  [[digital.localgov]] — extracted above: **[[tanzania--dpi-id]]** ([[nida-tanzania]] /
  [[jamii-namba]], [[rita-tanzania]], [[nao-tanzania]]) · **[[tanzania--gov-protect]]**
  ([[tanzania-pdpa]], [[pdpc-tanzania]]) · **[[tanzania--dpi-registry]]** ([[napa-tanzania]],
  [[upu-s42]], [[papu]]/[[upu]], [[tanzania-posts-corporation]]).
- [[dpi.pay]] · [[include.access]] — **[[tips-tanzania]]**, built in-house by
  **[[bank-of-tanzania]]** with **[[fsdt]]**/[[gates-foundation]] support; the
  **[[tanzania-financial-consumer-protection-regulations]]** (GN 298 of 2025) cross-reference the
  PDPA; the [[rswitch-rwanda]] corridor is the first EAC interlink. Fee caps + **113 licensed PSPs**
  (Mar 2026); **[[mpesa|M-Pesa]]** Fintech 2.0 and **[[nala]]** moving up the stack; a **mandatory
  cash-lite economy from 1 Jul 2026**. Full case: **[[tanzania--dpi-pay]]**.
- [[dpi.govtech]] · [[gov.policy]] — extracted: **[[tanzania--dpi-govtech]]** — World Bank **GTMI Group A**
  (top tier); **GovESB** interoperability backbone (PM-mandated); the **Digital Economy Strategic Framework
  2024–2034** targets; **ITU Acceleration Center**, **Huawei** and **IIT Madras Zanzibar** partners; startup
  ecosystem. Underpinned by **[[tanzania-e-government-strategy-2022]]** (2022–2027, the *second* such
  strategy; the e-Government Agency became a full **e-Government Authority in 2019** — [[ega-tanzania]]) and
  the **[[tanzania-digital-economy-framework]]** (2024–2034, [[micit-tanzania]], presidential foreword).
- [[infra.store]] · [[dpi.exchange]] — [[wingu-africa]]'s Mbezi Tier 3 phase 2 and the WCX platform;
  **[[liquid-intelligent-technologies|Liquid]]** entering; public data centres in Dodoma + Zanzibar; the
  **PDPA + localisation policy driving in-country hosting** (compliance as demand); the Digital Tanzania
  Project data-centre core; a second-tier DC market on 2031 projections. Full case: **[[tanzania--infra-store]]**.
- [[infra.connect]] · [[digital.rural]] · [[include.divides]] — **[[nictbb]]** state backbone (200→2,000
  Gbps; DRC link); **[[ttcl]]**'s **Kilimanjaro One** subsea cable to Mauritius; **[[yas-tanzania|Yas]]**
  (~4,800 towers, PPP handover of 758 towers) / Airtel / Vodacom; **5G at 32.83% pop but smartphones only
  42.5% and falling**; Helios Towers/UCSAF rural sites; [[tspar]]'s IXP extension to Wingu; Starlink live
  since Nov 2025. Full case: **[[tanzania--infra-connect]]**.
- [[dpi.mis]] · [[data.statistics]] — fragmented HMIS (18 systems), [[dhis2]] >90% of flows since
  2013, heaviest reporting burden of five studied countries; closed-source lock-in and donor power as
  structural constraints.
- [[gov.regional]] · [[tech.ai]] — EAC AI-sovereignty roadmap; UNECA DPI/sovereignty debate. National
  **AI story extracted**: **[[tanzania--tech-ai]]** ([[ict-commission-tanzania|ICTC]] AI Forum; AI a Vision
  2050 pillar; the "consumers-vs-contributors" debate; deepfakes; **digital health & AI** — hospitals
  exposed to ransomware, [[jakaya-kikwete-cardiac-institute|JKCI]] tooling).
- [[infra.cybersec]] — extracted: **[[tanzania--infra-cybersec]]** (ITU GCI tier-1; ICTC forum; KISA/South
  Korea skills; e-GA public-sector hardening; SIM-registration agents feeding fraud rings — cyber-governance
  ahead of cyber-hygiene).
- [[tech.innovate]] · [[finance.new]] — external ID financing; Google-backed startup Safiri.

*Naming note: the ministry appears as "Ministry of Information, Communication and Information
Technology" (2024 sources) and "Ministry of Communication and Information Technology" (2026 sources)
— the same body; variants recorded on [[micit-tanzania]], not silently normalised.*

## Reference studies held (cited, not absorbed)

**Digital Identity in Tanzania** ([[research-ict-africa]] with the Centre for Internet and Society;
Dr Patricia Boshe; 2021, date proxy — one of a ten-country study) · **Snapshot of CRVS Systems of
Tanzania Mainland** ([[idrc-crvs-centre-of-excellence]] with [[uneca]]; Yacob Zewoldi; 2019, built on
a [[rita-tanzania]] questionnaire of December 2018) · [[un-egdi]].

## Entities

- [[nida-tanzania]] · [[jamii-namba]] · [[rita-tanzania]] · [[pdpc-tanzania]] · [[tanzania-pdpa]] ·
  [[tanzania-infant-registration-bill]] · [[nao-tanzania]] · [[ministry-of-home-affairs-tanzania]]
- [[bank-of-tanzania]] · [[tips-tanzania]] · [[tanzania-financial-consumer-protection-regulations]] ·
  [[fsdt]] · [[gates-foundation]] · [[rswitch-rwanda]] · [[eac-cross-border-payment-masterplan]]
- [[micit-tanzania]] · [[tcra]] · [[ega-tanzania]] · [[napa-tanzania]] ·
  [[tanzania-e-government-strategy-2022]] · [[tanzania-digital-economy-framework]] ·
  [[jamii-x-change]] · Jamii Kadi · Jamii Malipo · Jamii Portal ·
  [[digital-tanzania-project]]
- [[tanzania-posts-corporation]] · [[papu]] · [[upu]] · [[upu-s42]] · [[tspar]]
- [[wingu-africa]] · [[anton-volianskyi]] · [[helios-towers]] · [[starlink]] · [[spacex]] ·
  [[airtel-africa]] · [[vodacom]] · [[starlink-airtel-d2c-partnership]] ·
  [[starlink-vodacom-leo-partnership]]
- [[james-kaji]] · [[edson-guyai]] · [[unicef]] · [[id4africa]] · [[world-bank]] · [[giz]] ·
  [[uneca]] · [[eac]] · [[usaid]] · [[research-ict-africa]] · [[idrc-crvs-centre-of-excellence]] ·
  [[un-egdi]] · [[baker-mckenzie]] · [[clyde-and-co]]
- [[hmis-tanzania-pea]] · [[rhis-recording-burden-5country]] · [[dhis2]] ·
  [[google-for-startups-accelerator-africa]] · [[google]]
- Mentioned, not hubbed (hub-level items): President Samia Suluhu Hassan; Angela Kairuki (ICT
  Minister); Pius Stephen Chaya (Deputy Minister); Noah Maina (TSPAR); Nestory Nicholas Maro (BoT);
  Safiri; UCSAF; Kassim Majaliwa (PM); Jerry William Silaa (ICT Minister, 2024); Mohammed Khamis
  Abdulla (PS, MICIT); Benedict Ndomba (e-GA DG); John Sausi (Finance & Planning); Zanzibar ID. *(Named figures from the extracted identity, data-protection and addressing material
  are listed on their intersections.)*
