---
type: place
title: South Sudan
code: SSD
parent: XEA
place_kind: country
topics: [dpi.pay, dpi.govtech, dpi.id, dpi.registry, infra.connect, infra.energy, infra.store, geopol.china, gov.protect, gov.legislate, gov.policy, finance.new, include.divides, include.access, tech.industry]
status: active
last_reviewed: 2026-07-16
---

# South Sudan (SSD)

South Sudan is digitising faster than the infrastructure and governance meant to
support it. A **de-facto DPI** — telecom mobile money, donor biometric registries,
humanitarian cash systems, a privately-owned tax platform — keeps commerce and
services moving, but it is fragmented, proprietary, and vulnerable to outages,
insecurity and political interference. The two structural gaps are a **reliable
connectivity/energy layer** and, most critically, the **absence of a unified
interoperable national digital ID**. A wave of late-2025/early-2026 reforms —
instant payments, a full-digitisation decree, gateway/data-centre oversight, and
draft data-protection and cybercrime laws — is building the missing scaffolding,
but ahead of the governance frameworks meant to hold it.

## Recent developments

- **2026-01-20** — **"What is actually functioning as DPI."** A Sudans Post
  synthesis (documenting Oct 2025–Jan 2026) of the country's de-facto DPI: the
  Bank of South Sudan's **IBPSS** (Oct 2025) as the first interoperable interbank
  backbone; telecom **mobile money** (>1m users) degrading to cash during outages;
  a **Nimule one-stop border post** with customs automation; a **primary
  tax/revenue platform privately owned by Adut Salva Kiir Mayardit**, the
  president's daughter; a **World-Bank-backed biometric public-servant payroll**
  that stays siloed absent a national ID; and donor WHO/UNICEF health systems as
  the most reliable-but-proprietary layer. Reinforces the 2026-01-30 feature
  below. Source: [[2026-01-20 What is actually functioning as DPI in South Sudan]].
  See [[dpi.pay]], [[dpi.exchange]], [[dpi.govtech]], [[include.divides]].
- **2026-01-17** — **NCA gateway & data-centre oversight committee.** The
  **[[national-communication-authority-south-sudan]]** (DG Rizig Dominic Samuel)
  formed a **Gateway Services and Data Centre Oversight Committee** with Swiss
  operator **[[mgi-communications]]**, to digitise **SSIGW** gateway revenue,
  modernise infrastructure and bring gateway + national data centre under national
  sovereignty/security oversight. A foreign private operator holding both the
  national gateway and data centre is a notable sovereignty concentration.
  Source:
  [[2026-01-17 South Sudan NCA establishes gateway services and data centre oversight committee]].
  See [[infra.store]], [[infra.connect]], [[gov.legislate]].
- **2026-01-16** — **Passport & ID issuance resumes.** The
  **[[dcrnpi-south-sudan]]** (DG Maj. Gen. Elia Kosta Faustino) resumed issuing
  passports, nationality certificates and national ID after months of booklet
  shortages. The **personal ID (Butaka Shakhsia)** — not the nationality
  certificate — is now required for **banking, land and court** (adults 18+);
  380,000 personal ID cards in stock. A **SIM-linked foreigner registration** was
  announced. Net-new to the already-held 2026-01-19 NRA cashless-payment reform.
  Source: [[2026-01-16 Juba - Passport, ID card issuance resumes]]. See
  [[dpi.id]], [[dpi.registry]], [[south-sudan--dpi-id]].
- **2026-01-19** — **Cashless digital passport payments.** The
  **[[south-sudan-national-revenue-authority]]** (Commissioner-General William Anyuon
  Kuol) abolished cash at immigration offices in Juba, replacing it with an
  EU-supported bank-linked digital payment system for passports and national ID —
  invoices auto-linked to the immigration database — to curb revenue leakage and a
  black market in passport booklets. Follows a months-long booklet shortage (10,000
  received after clearing supplier debts) and a **Nov 2025 biometric registration of
  public servants** (anti-ghost-worker). Analysts stress the reform is limited absent
  a trusted national digital ID layer. Source:
  [[2026-01-19 South Sudan introduces digital payment system for passports after supply crisis]].
  See [[dpi.pay]], [[dpi.govtech]], [[dpi.id]], [[gov.protect]].
- **2026-01-30** — **"Can a digital economy exist without reliable DPI?"** Feature
  on the country's de-facto DPI: mobile money is the primary payments layer but a
  broken acceptance chain (outages, liquidity shortages) reinforces cash hoarding.
  The **[[bank-of-south-sudan]]**'s **Inter-Bank Payment & Settlement System**
  (launched Oct 2025) is undermined by incomplete bank integration and governance
  concerns that key tax/revenue systems are individually rather than institutionally
  controlled. Internet is among the most expensive in the region; **[[starlink]]**
  relieves some NGOs/media but is beyond most households. Humanitarian/health
  biometric registries are the most reliable systems yet remain donor-driven,
  proprietary and disconnected from national registries. Draws a comparison with
  **[[KEN]]**. Source:
  [[2026-01-30 Can a digital economy exist in South Sudan without reliable DPI]]. See
  [[dpi.pay]], [[dpi.exchange]], [[include.divides]].
- **2025-12-19** — **Full digitisation of public institutions ordered.** President
  Salva Kiir decreed full authority to the
  **[[electronic-services-council-south-sudan]]** (chair Thomas Gatkuoth Nyak) to
  digitise all government institutions, with a **January 2026** adoption deadline;
  claimed ~40% economic improvement since e-services began. South Sudan ranks
  **192/193 (2024)** on the UN E-Government Development Index (EGDI 0.1191). Source:
  [[2025-12-19 South Sudan orders full digitization of public institutions]]. See
  [[dpi.govtech]], [[gov.policy]].
- **2025-12-15** — **Civil registration / statelessness baseline.** As of late
  2025 only ~**10%** of the ~12m population held Nationality Certificates and
  national ID/birth certificates were not yet issued at scale; a **CRVS Costed
  Plan** and Civil Registry Rules are adopted, with a birth-registration pilot at
  Juba Teaching Hospital (Hold the Child + UNHCR/UNICEF/NRC/IOM/UNECA/Vital
  Strategies). Source:
  [[2025-12-15 Strengthening Civil Registration and Ending Statelessness in South Sudan]].
  See [[dpi.id]], [[include.access]], [[south-sudan--dpi-id]].
- **2025-12-11** — **First Data Protection Bill promised for 2026.** ICT minister
  Ateny Wek Ateny said South Sudan will introduce its first-ever
  **[[south-sudan-data-protection-bill]]** in 2026 to complement the
  **[[south-sudan-cybercrime-bill-2025]]** (passed, awaiting assent). Activist
  Edmund Yakani warned the cybercrime law risks being abused without training and
  reform, amid rising digital-facilitated GBV. Would align with Kenya's DPA and
  the AU Malabo Convention. Source:
  [[2025-12-11 South Sudan to get data protection bill in 2026, says minister]]. See
  [[gov.protect]], [[gov.legislate]], [[infra.cybersec]].
- **2025-10-17** — **Data-protection convening in Juba.** Paradigm Initiative and
  **[[koneta-hub]]** (ED Denaya Dennis) hosted a two-day forum on the draft Data
  Protection Bill; the NCA's Chol Joseph Mayen keynoted. Aim: improve the draft and
  secure a parliamentary commitment to table it. Source:
  [[2025-10-17 Paradigm Initiative partners with Koneta Hub on data protection law in South Sudan]].
  See [[gov.protect]], [[gov.legislate]].
- **2025-02-07** — **First National Instant Payment System (NIPS).** The
  **[[bank-of-south-sudan]]** launched **[[nips-south-sudan]]** with the
  **[[africanenda]]** Foundation (governor Johnny Ohisa Damian, 5 Feb 2025) —
  real-time interoperable P2P/P2B/G2P/P2G rails integrating ACH/RTGS/IFT, aimed at
  6m+ under-banked adults, aligned to the EAC instant-payments master plan.
  Predates the Oct-2025 IBPSS; whether the two are one programme, two phases or
  distinct rails is unresolved (see below). Source:
  [[2025-02-07 South Sudan launches first national instant payment system (NIPS)]].
  See [[dpi.pay]], [[gov.regional]], [[include.access]].

## Active topics

- [[dpi.pay]] · [[dpi.govtech]] — telecom mobile money as the de-facto payments
  layer (fragile, cash-hoarding); **NIPS** (Feb 2025, BoSS + AfricaNenda, retail
  instant payments) and the **IBPSS** (Oct 2025, interbank settlement) — possibly
  phases of one programme or two rails; NRA cashless digital passport/ID payments;
  full-digitisation decree (Jan 2026 deadline); mobile money declared legal tender (Jul 2025), RTGS uptake, ITU DFS Security Lab. Full case: [[south-sudan--dpi-pay]]. **Open reconcile:** NIPS vs IBPSS.
- [[dpi.id]] · [[dpi.registry]] · [[gov.protect]] — **no unified interoperable
  national digital ID** (the defining gap); personal ID (Butaka Shakhsia) now
  gating banking/land/court; ~10% Nationality-Certificate coverage; EU/UNHCR
  returnee ID issuance; CRVS Costed Plan + birth-registration pilot; siloed
  biometric payroll and donor humanitarian registries. Full treatment:
  **[[south-sudan--dpi-id]]**.
- [[gov.protect]] · [[gov.legislate]] · [[infra.cybersec]] — first **Data
  Protection Bill** promised for 2026; **Cyber Crimes and Computer
  Abuse Act 2026** signed by Kiir (18 Feb 2026, in effect); no comprehensive data-protection regime yet
  (Data Protection Policy Bill drafted Oct 2025, Koneta/Paradigm/NCA). Full case: [[south-sudan--gov-protect]].
- [[infra.store]] · [[infra.connect]] — NCA/MGI **gateway & national data-centre
  oversight committee** (Jan 2026), digitising SSIGW revenue under a
  sovereignty/security mandate; among the region's most expensive internet;
  coverage 69%→80% (2023→2026) via CREI solar-hybrid at MTN sites; Starlink for
  NGOs/media only; first national fibre backbone (MTN/Bayobab 15-yr licence + $9m design/2,400km
  Kenya cable), MTN eSIM/25%-cut, ~$23.70/GB, FX tariff crisis, Jan-2025 social-media shutdown. Full
  case: [[south-sudan--infra-connect]]. **Connectivity baseline (early 2025):** 4.47m mobile
  connections (37.1%), 1.90m internet users (15.7%), 868k social (7.2%), pop 12.1m
  — [[2025-03-03 Digital 2025 - South Sudan (DataReportal)]].
- [[infra.energy]] · [[include.divides]] · [[include.access]] — reliable energy is
  the binding constraint on connectivity; sharp urban (Juba) bias; rural and
  border counties digitally excluded; legal identity as the gateway to services.
- [[geopol.china]] · [[tech.industry]] — Huawei re-engagement (post-2014 rift);
  Huawei/ZTE legacy footprint.

## Entities

- [[dcrnpi-south-sudan]] — civil registry / national ID / passport authority
- [[bank-of-south-sudan]] · [[nips-south-sudan]] · [[south-sudan-national-revenue-authority]]
  — state payment/revenue institutions and rails
- [[national-communication-authority-south-sudan]] · [[mgi-communications]] —
  ICT regulator + Swiss gateway/data-centre operator
- [[electronic-services-council-south-sudan]] — e-government / digitisation body
- [[south-sudan-data-protection-bill]] · [[south-sudan-cybercrime-bill-2025]] —
  forthcoming/pending digital-governance instruments
- [[koneta-hub]] · [[paradigm-initiative]] · [[africanenda]] — civil-society /
  digital-rights / financial-inclusion partners
- [[crei-south-sudan]] · [[finnfund]] · [[mtn-group]] — telecom energy financing
- [[huawei]] · [[zte]] · [[starlink]] — foreign infrastructure/connectivity
- Mentioned, not hubbed: WHO, UNICEF, UNHCR, EU (donor DPI); Ateny Wek Ateny (ICT
  minister); Thomas Gatkuoth Nyak (e-services council); Adut Salva Kiir Mayardit
  (private tax platform); Edmund Yakani (civil society).
