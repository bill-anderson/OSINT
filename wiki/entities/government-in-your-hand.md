---
type: entity
entity_type: initiative
title: Government in Your Hand (GIYH)
places: [SWZ]
topics: [dpi.govtech, dpi.id, dpi.exchange, dpi.pay, include.access]
lens: [sovereignty]
status: active
last_reviewed: 2026-07-17
sources: [[2025-01-01 ToR - Government in Your Hand Digital Government Implementation Roadmap (Digital Eswatini)], [2024-12-05 Eswatini Advances Toward Digital Transformation - Key Findings from the Digital Readiness Assessment], [2024-12-10 Unveiling Eswatini's Digital Path], [2024-11-22 Eswatini unveils readiness report for digital ID, transformation push], [2025-08-05 Robust DPI inevitable for Eswatini's economic, jobs boom - World Bank], [2025-01-22 Eswatini partners with Google to drive digital transformation and economic growth], [2026-05-18 Eswatini promotes digital transformation project through World Bank engagements]]
---

# Government in Your Hand (GIYH)

**Eswatini's digital-government flagship** — both the theme of the
[[eswatini-digitalisation-strategy-2024-2028]] and the delivery platform meant to put
public services on citizens' phones. Owned by [[ministry-of-ict-eswatini]]; its
implementation roadmap is scoped under the [[world-bank]]
[[digital-eswatini-project]]. See [[SWZ]], [[dpi.govtech]].

## Activity in this wiki

- **Scope (ToR, 2025, year precision).** The Digital Government Implementation Roadmap
  covers **digital ID, data exchange, social registries, PKI, payments and ~200
  services**. See [[dpi.id]], [[dpi.exchange]], [[dpi.pay]]. Source:
  [[2025-01-01 ToR - Government in Your Hand Digital Government Implementation Roadmap (Digital Eswatini)]].
- **The app launched May 2025 — and drew privacy criticism.** Eswatini launched the
  **GIYH App in May 2025**; the move "faced criticism over data privacy issues" **(as
  reported 2025-08-05)**. The wiki holds no source setting out the substance of that
  criticism or any government response — a provenance gap on the country's most visible
  digital service. See [[gov.protect]], [[sovereignty]]. Source:
  [[2025-08-05 Robust DPI inevitable for Eswatini's economic, jobs boom - World Bank]].
- **Service ambition (2026-05).** The [[eswatini-digital-transformation-project]]
  negotiations frame GIYH as the channel for passport applications, business
  registration, vehicle licensing, health services, education systems and social
  services. See [[include.access]]. Source:
  [[2026-05-18 Eswatini promotes digital transformation project through World Bank engagements]].
- **Aligned to the DRA (2024-11-22).** [[savannah-maziya]]: "It is encouraging that the
  findings of this DRA are aligned with the Government in Your Hand Strategy" — though
  the [[eswatini-digital-readiness-assessment]] itself found DPI, connectivity and
  regulation lagging, and **half of stakeholders (2024)** perceiving strategy
  implementation as delayed. Sources:
  [[2024-11-22 Eswatini unveils readiness report for digital ID, transformation push]],
  [[2024-12-05 Eswatini Advances Toward Digital Transformation - Key Findings from the Digital Readiness Assessment]].
- **Google tooling (2025-01-22).** The [[eswatini-google-partnership]] is aimed at
  integrating [[google]]'s tools into GIYH — on a single secondary source; see that
  page. Source:
  [[2025-01-22 Eswatini partners with Google to drive digital transformation and economic growth]].

## Sources

- [[2025-01-01 ToR - Government in Your Hand Digital Government Implementation Roadmap (Digital Eswatini)]]
- [[2024-12-05 Eswatini Advances Toward Digital Transformation - Key Findings from the Digital Readiness Assessment]]
- [[2024-12-10 Unveiling Eswatini's Digital Path]]
- [[2024-11-22 Eswatini unveils readiness report for digital ID, transformation push]]
- [[2025-08-05 Robust DPI inevitable for Eswatini's economic, jobs boom - World Bank]]
- [[2025-01-22 Eswatini partners with Google to drive digital transformation and economic growth]]
- [[2026-05-18 Eswatini promotes digital transformation project through World Bank engagements]]
