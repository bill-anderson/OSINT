---
type: entity
entity_type: company
title: Orange
places: [XWA, XAF, NGA, SEN, CIV, MAR]
topics: [infra.connect, geopol.eu, capacity.training, infra.store]
lens: [colonialism]
status: active
last_reviewed: 2026-07-12
sources: [[2026-05-23 Orange-backed subsea cable to connect Nigeria, 19 other countries], [2024-05-30 Orange and AWS partner to deliver cloud services in Morocco and Senegal], [2026-01-21 Orange Côte d'Ivoire Launches Orange Sat Satellite Internet to Boost Nationwide Connectivity], [2025-12-04 Is it a bird Is it plane No, Jim, it's an African mobile operator, but not as we know it 9], [2026-02-16 Waking the dead - Slow road to extinction for Africa's former telco incumbents 11]]
---

# Orange

French telecom operator with deep incumbency across West and Central Africa, extending
its footprint through subsea infrastructure and skills programmes — a European carrier
whose expansion into African [[infra.connect]] carries a [[geopol.eu]] and colonial
legacy dimension.

## Activity in this wiki

- **Leads the Via Africa subsea cable consortium (announced 2026-05-12, reported 2026-05-23).**
  Orange heads the consortium behind [[via-africa-cable]], a >20,000km Atlantic corridor
  connecting West Africa to Europe, with expected landings in [[NGA]], [[SEN]], Guinea,
  [[CIV]] and Mauritania.
  Source: [[2026-05-23 Orange-backed subsea cable to connect Nigeria, 19 other countries]].
- **Skills and startup pledges ([[capacity.training]], to 2030).** Orange pledged to
  train >3m African youth in AI, cybersecurity and cloud by 2030, to expand its Orange
  Digital Centres from 50 to 100 across Africa and the Middle East, and to back 500+
  startups.
  Source: [[2026-05-23 Orange-backed subsea cable to connect Nigeria, 19 other countries]].
- **AWS Wavelength Zones, Morocco & Senegal (backfill, 2024-05).** Orange Middle East &
  Africa partnered with [[amazon-web-services]] to bring **AWS Wavelength Zones** to [[MAR]]
  and [[SEN]] — the first in Africa and first in countries without an AWS Region — for
  data-residency and low-latency workloads, with Orange as anchor customer hosting the compute
  in its data centres. Historical baseline for Orange's Africa cloud footprint. See
  [[infra.store]], [[geopol.eu]]. Source:
  [[2024-05-30 Orange and AWS partner to deliver cloud services in Morocco and Senegal]].
- **Orange Sat satellite broadband, Côte d'Ivoire (2026-01).** [[orange-cote-divoire]]
  launched [[orange-sat]], a [[eutelsat]]-backed satellite-internet service targeting rural
  "white areas" beyond 4G and fibre, complementing its fibre/mobile portfolio. See [[CIV]],
  [[infra.connect]], [[include.divides]]. Source:
  [[2026-01-21 Orange Côte d'Ivoire Launches Orange Sat Satellite Internet to Boost Nationwide Connectivity]].

- **Rural DRC solar base-station JV (Russell Southwood, 2025-12-04).** Orange and
  [[vodacom]] signed a **Jan 2025** joint venture to build **solar-powered base
  stations** in rural **[[COD]]** — an example of the network-sharing and
  off-grid energy models Southwood argues MNOs must adopt to bear rural CAPEX.
  Orange's mobile-money share is **20.4%** of EMEA revenue. See [[COD]],
  [[infra.energy]], [[tech.industry]]. Source:
  [[2025-12-04 Is it a bird Is it plane No, Jim, it's an African mobile operator, but not as we know it 9]].
- **Telkom Kenya exit (Russell Southwood, 2026-02-16).** In Southwood's account of
  Africa's stranded former incumbents, Orange bought **60%** of Telkom Kenya for
  **US$390m**, raised it to **70%** in 2013, then sold **60%** to **Helios
  Investment Partners** (private equity — not [[helios-towers]]) in 2016; Helios
  later sold back for just **US$46.85m** against **US$291.12m** of liabilities —
  a case study in failed incumbent privatisation. See [[KEN]], [[tech.industry]].
  Source: [[2026-02-16 Waking the dead - Slow road to extinction for Africa's former telco incumbents 11]].

## Sources

- [[2025-12-04 Is it a bird Is it plane No, Jim, it's an African mobile operator, but not as we know it 9]]
- [[2026-02-16 Waking the dead - Slow road to extinction for Africa's former telco incumbents 11]]
- [[2026-05-23 Orange-backed subsea cable to connect Nigeria, 19 other countries]]
- [[2024-05-30 Orange and AWS partner to deliver cloud services in Morocco and Senegal]]
- [[2026-01-21 Orange Côte d'Ivoire Launches Orange Sat Satellite Internet to Boost Nationwide Connectivity]]
