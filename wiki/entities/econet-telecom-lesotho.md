---
type: entity
entity_type: company
title: Econet Telecom Lesotho (ETL)
places: [LSO]
topics: [infra.connect, dpi.pay, tech.ai]
lens: []
status: active
last_reviewed: 2026-07-18
sources: [[[2025-06-20-econet-5g-network-upgrade-techafricanews]], [[2025-05-21-econet-ran-network-upgrade-lestimes]], [[2025-07-11-econet-wifi-plus-fmc-lestimes]], [[2025-03-11-starlink-opposition-section2-connectingafrica]]]
---

# Econet Telecom Lesotho (ETL)

A Lesotho **mobile network operator** — **70% [[econet-group|Econet Wireless]] Global, 30% Government
of Lesotho** (a local-ownership precedent cited in the Starlink debate). Operates the **EcoCash** mobile-
money platform. CEO Dennis Plaatjies. See [[infra.connect]], [[lesotho--infra-connect]].

## Activity in this wiki

- **Network upgrades — RAN, 5G, Wi-Fi (2025).** [[infra.connect]] [[tech.ai]] Under the **"Re le Utloile"**
  campaign, ETL ran a phased **Maseru RAN/transport upgrade** (from Feb 2025), grew **5G sites 54→149 on
  700MHz** with AI-driven real-time network operations (reaching Leribe, Mafeteng, Quthing, Butha-Buthe),
  and launched **Wi-Fi Plus** fixed-mobile convergence for residential and SMME customers. Sources:
  [[2025-05-21-econet-ran-network-upgrade-lestimes]], [[2025-06-20-econet-5g-network-upgrade-techafricanews]],
  [[2025-07-11-econet-wifi-plus-fmc-lestimes]].
- **Ownership precedent (2025-03).** ETL's **70/30** split (with Vodacom's 80/20) featured in the
  Starlink 100%-foreign-ownership debate. See [[lesotho--infra-connect]]. Source:
  [[2025-03-11-starlink-opposition-section2-connectingafrica]].
- **EcoCash upgrade via SEFS (2025-04).** [[dpi.pay]] ETL's **Sasai Econet Financial Services (SEFS)**
  migrated the **[[ecocash|EcoCash]]** mobile-money platform to a new system (Oct 2024), adding Mokhatlo,
  payroll, fraud controls and wallet interest. See [[lesotho--dpi-pay]]. Source:
  [[2025-04-30-econet-ecocash-upgrade-lestimes]].

## Related

- [[LSO]] · [[infra.connect]] · [[dpi.pay]] · [[econet-group]] · [[ecocash]] · [[lesotho--infra-connect]] · [[lesotho--dpi-pay]]
