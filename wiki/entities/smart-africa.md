---
type: entity
entity_type: organisation
title: Smart Africa (Smart Africa Alliance)
places: [XAF, RWA, GHA, DJI, GIN, MAR, MLI]
topics: [infra.connect, gov.regional, gov.standards, capacity.training, tech.ai, dpi.exchange, infra.cybersec, gov.policy, gov.protect]
lens: [sovereignty, colonialism]
status: active
last_reviewed: 2026-07-22
sources: [[2026-03-10-techafricanews-mali-smart-africa-mwc2026], [2026-07-09 Smart Africa and AFNIC Partner to Strengthen Africa's Internet Infrastructure], [2026-05-28 Building the same road twice], [2026-05-28 Estonia backs Africa digital infrastructure], [2026-04-27 Smart Africa drives cross-border data interoperability through SADX Technical Mission], [2026-04-16 Smart Africa, RealTyme join forces for data sovereignty], [2026-04-03 Ghana approves $250M AI Hub & National AI Strategy], [2026-05-21 Pushing back from Big Tech Africa's hard road to AI sovereignty], [2026-03-16 New $137M initiative to enable digital service access for millions in three ECOWAS nations], [2025-06-18 Benin envisages mobile wallet launch in inclusive national digital ID push], [2025-01-29 Deadline extended for Benin digital government transformation tender], [2025-11-04 The Double Standard Why Are Internet Governance Rules Different for Africa], [2026-07-13 Djibouti Seeks ITU, Smart Africa Support for Telecoms and AI Development], [2024-07-02 Ghana develops its National Data Strategy with Smart Africa and Team Europe], [2025-11-13 Transform Africa Summit 2025 opens in Conakry, epicentre of digital cooperation in Africa], [2025-11-19 Transform Africa Summit 2025 (Conakry) - Simandou 2040 Pillar 3, TELEMO, SANIA-SADX and the closing wrap], [2025-09-09 Get to know The Republic of Guinea Home of Transform Africa Summit 2025], [2025-07-14 Smart Africa co-creates Africa's first Cross-Border Data Exchange Guidelines (Rabat, ADD-chaired CAITA)], [2023-11-07-worldbank-dtfa-wardip-sop1-pad4756], [2026-07-16-lesotho-national-data-value-creation-blueprint-workshop], [2025-11-17-smart-africa-inaugural-africa-ai-council]]
---

# Smart Africa (Smart Africa Alliance)

Pan-African alliance of member states, international organisations and private-sector
partners advancing the continent's digital single market and infrastructure (CEO **Lacina
Koné**; secretariat in Kigali, [[RWA]]). A **bloc/programme entity**, not a place. Long
listed as a "mentioned" body — hubbed here as it becomes the subject of a source.

## Activity in this wiki

- **Africa's first Cross-Border Data Exchange Guidelines (Rabat, 2025-07).** [[dpi.exchange]] Through its
  **Council of African IT Agencies (CAITA)** — chaired by Morocco's Digital Development Agency
  ([[add-morocco|ADD]]) — Smart Africa convened 11 member states in Rabat to draft the continent's first
  **Cross-Border Data Exchange Guidelines** under the Smart Africa Trust Alliance (SATA), exploring the
  GovStack Information Mediator building block. Participants: Benin, Burkina Faso, CIV, Comoros, Gabon,
  Ghana, Mauritania, Nigeria, Tunisia, Zambia, Morocco. See [[MAR]], [[gov.standards]]. Source:
  [[2025-07-14 Smart Africa co-creates Africa's first Cross-Border Data Exchange Guidelines (Rabat, ADD-chaired CAITA)]].
- **Hosted Transform Africa Summit 2025 in Conakry (2025-11).** [[gov.regional]] [[tech.ai]] The 7th
  **[[transform-africa-summit-2025|Transform Africa Summit]]** ("AI for Africa"), with the Government of
  **[[GIN]] Guinea** — the first Francophone West African host — drew 7,000+ participants and 14 MoUs;
  Smart Africa unveiled **SANIA** (innovation-ecosystem platform) and signed MoUs (incl. **[[visa]]**,
  YouthConnekt, a **SADX** pilot for Benin/Ghana/Rwanda, SADA with The Gambia, AFTLD). Smart Africa is
  now **42 member states** (from 37) with a new three-year strategy (flagships SANIA, SADX, Digital ID).
  Guinea joined the Alliance in 2016 and, through its regulator **[[arpt-guinea|ARPT]]**, chairs the
  Smart Africa **Council of African Regulators (CAR)**. See [[GIN]], [[XAF]]. Sources:
  [[2025-11-13 Transform Africa Summit 2025 opens in Conakry, epicentre of digital cooperation in Africa]],
  [[2025-11-19 Transform Africa Summit 2025 (Conakry) - Simandou 2040 Pillar 3, TELEMO, SANIA-SADX and the closing wrap]],
  [[2025-09-09 Get to know The Republic of Guinea Home of Transform Africa Summit 2025]].
- **AFNIC MoU (2026-07, Geneva).** Smart Africa signed a Memorandum of Understanding with
  **[[afnic]]** (France's domain registry) to strengthen Africa's internet infrastructure:
  building the capacity of African **ccTLD operators**, hardening **DNS** security and
  resilience, modernising national domain registries, and improving internet governance.
  Signed by Lacina Koné and AFNIC DG Pierre Bonis. See [[smart-africa-afnic-mou]],
  [[infra.connect]], [[infra.cybersec]], [[gov.standards]], [[XAF]]. Source:
  [[2026-07-09 Smart Africa and AFNIC Partner to Strengthen Africa's Internet Infrastructure]].
- **Smart Africa Data Exchange (SADX) (2026-07).** Launched the **SADX** cross-border
  data-exchange (pilot **Benin, Ghana, Rwanda**) — which the Data Landscapers "building the
  same road twice" analysis pairs against the AfCFTA Secretariat's competing **ADAPT**
  ([[afcfta]]) as wasteful duplication. A prior **SADX Technical Assessment Mission** (Kigali,
  Apr 2026) with the same three states worked the build-out — PKI trust chains, API contracts,
  cross-border verification-request processing — toward a "functional, federated and
  decentralised" architecture ([[sadx]]). See [[dpi.exchange]], [[gov.regional]]. Sources:
  [[2026-05-28 Building the same road twice]],
  [[2026-04-27 Smart Africa drives cross-border data interoperability through SADX Technical Mission]].
- **ESTDEV MoU (2026-05-28, Brussels).** Signed an MoU with Estonia's development agency
  **[[estdev]]** ([[estdev-smart-africa-mou]]) to build interoperable, reusable DPI across its
  **42 member states**, reduce vendor lock-in and strengthen cybersecurity and institutional
  capacity. See [[dpi.exchange]], [[geopol.eu]], [[finance.mou]]. Source:
  [[2026-05-28 Estonia backs Africa digital infrastructure]].
- **RealTyme partnership (2026-04-16).** The Smart Africa Digital Academy partnered with Swiss
  zero-trust firm **[[realtyme]]** ([[smart-africa-realtyme-partnership]]) on a "national
  protection framework" — sovereign communication enclaves explicitly positioned against the
  **US Cloud Act**, framed as "digital decolonisation" (claiming Africa faces a ~60% higher
  cyber-attack rate than the global average). See [[infra.cybersec]], [[gov.protect]]. Source:
  [[2026-04-16 Smart Africa, RealTyme join forces for data sovereignty]].
- **Africa AI Council — formally established 2025-11-17 (Conakry).** [[tech.ai]] [[gov.regional]] The
  Smart Africa **Board** formally established the **[[africa-ai-council|Africa AI Council]]** at Transform
  Africa Summit, naming **15 members** (7 African ICT ministers + 8 independents) across six thematic areas,
  selected by Smart Africa/AUC/ITU from 400+ applications. The body was **endorsed a year earlier** by the
  Kigali **[[africa-ai-declaration|Africa AI Declaration]]** (Apr 2025), which placed it under the Smart Africa
  Steering Committee **co-chaired by the AUC and [[itu|ITU]]**; the Board that unveiled it is Kagame-chaired.
  Part of the continental AI-sovereignty push ($60bn Africa AI Fund, Apr 2025 Kigali). See [[XAF]]. Source
  (primary): [[2025-11-17-smart-africa-inaugural-africa-ai-council]]; also
  [[2026-05-21 Pushing back from Big Tech Africa's hard road to AI sovereignty]].
- **CAIGA — internet-governance capture critique (Munyua, 2025-11-04).** Munyua ("The Double
  Standard") charges that Smart Africa's **Continental Africa Internet Governance Architecture
  ([[caiga]])** — co-developed and funded by **[[icann]]** under a 2024 ICANN/Smart Africa MoU, with a
  blueprint drafted by a third party based in France — would let the Smart Africa Heads-of-State
  Summit **politically endorse [[afrinic]] governance reforms**, install a Smart Africa-provided
  Permanent Secretary, and substitute political endorsement for member ratification: i.e. governmental
  capture of a community-governed regional registry. Presented at ICANN84 Dublin as "two years of
  joint work with ICANN," and at the Africa Internet Summit Accra with 24 hours' notice. Smart Africa
  given here as **40 member states** (cf. 42 elsewhere on this page). See [[gov.regional]],
  [[infra.connect]], [[sovereignty]]. Source:
  [[2025-11-04 The Double Standard Why Are Internet Governance Rules Different for Africa]].
- **Ghana National Data Strategy partner (2024-07-02).** [[gov.policy]] With Team Europe (EU, GIZ,
  D4D Hub, Global Gateway), Smart Africa co-facilitated the National Validation Workshop for the
  **[[ghana-national-data-strategy]]** (Accra) — aligned to the AU Continental Data Policy Framework,
  with Ghana's [[data-protection-commission-ghana|DPC]] a lead stakeholder and a Benin peer-learning
  exchange. CEO Lacina Koné framed data as a cross-sector asset. See [[GHA]], [[dpi.exchange]],
  [[gov.protect]], [[gov.regional]]. Source:
  [[2024-07-02 Ghana develops its National Data Strategy with Smart Africa and Team Europe]].
- **Ghana National AI Strategy partner (2026-04).** Ghana's Cabinet approved its
  **National AI Strategy (2023–2033)** and a **$250m** national AI computing centre; the
  strategy was developed with **Smart Africa**, GIZ FAIR Forward and The Future Society,
  with public launch set for 24 Apr 2026. See [[ghana-national-ai-strategy]], [[GHA]],
  [[tech.ai]], [[capacity.training]], [[gov.policy]]. Source:
  [[2026-04-03 Ghana approves $250M AI Hub & National AI Strategy]].
- **WARDIP2 collaborator (2026-03).** Named a partner — with **[[ecowas]]**, the **[[african-union]]**
  and the **[[world-bank]]** — on **[[wardip]]**, the **$137m** Western Africa Regional Digital
  Integration Program phase 2 (Benin, Liberia, Sierra Leone), targeting broadband and digital
  public services for ~10.6m people and a common regional digital market. See [[gov.regional]],
  [[infra.connect]], [[XWA]]. Source:
  [[2026-03-16 New $137M initiative to enable digital service access for millions in three ECOWAS nations]].
  (Smart Africa's **Digital ID Blueprint** and cross-border trust-framework work also underpin Benin's
  national-ID and e-gov programmes.)

- **Mali cooperation at MWC 2026 (2026-03-03).** [[gov.regional]] [[capacity.training]] On the
  sidelines of MWC 2026 (Barcelona), Mali's digital minister Alhamdou Ag Ilyène met CEO Lacina
  Koné to reaffirm a strengthened Smart Africa partnership — human-capital/skills development,
  reinforcing fibre-optic interconnections with neighbours, and Mali possibly hosting the next
  **Transform Africa Summit** — signalling Mali balancing AES-centric sovereignty with
  continental cooperation. See [[MLI]], [[XWA]]. Source:
  [[2026-03-10-techafricanews-mali-smart-africa-mwc2026]].
- **Djibouti alignment sought (2026-07).** At WSIS 2026, CEO Lacina Koné met Djibouti's
  communications minister to discuss aligning **[[DJI]] Djibouti**'s national digital projects
  (the Djibouti Digital Foundation) and the **[[eardip]]** regional integration project with
  Smart Africa's continental data-sovereignty and AI initiatives. See [[gov.regional]],
  [[tech.ai]], [[XEA]]. Source:
  [[2026-07-13 Djibouti Seeks ITU, Smart Africa Support for Telecoms and AI Development]].

## Related

- [[afnic]] · [[smart-africa-afnic-mou]] · [[eardip]] · the continental digital-single-market
  thread ([[afcfta]], [[gov.regional]]).

## Sources

- [[2026-03-10-techafricanews-mali-smart-africa-mwc2026]]
- [[2026-07-09 Smart Africa and AFNIC Partner to Strengthen Africa's Internet Infrastructure]]
- [[2026-05-28 Building the same road twice]]
- [[2026-05-28 Estonia backs Africa digital infrastructure]]
- [[2026-04-27 Smart Africa drives cross-border data interoperability through SADX Technical Mission]]
- [[2026-04-16 Smart Africa, RealTyme join forces for data sovereignty]]
- [[2026-05-21 Pushing back from Big Tech Africa's hard road to AI sovereignty]]
- [[2026-04-03 Ghana approves $250M AI Hub & National AI Strategy]]
- [[2026-03-16 New $137M initiative to enable digital service access for millions in three ECOWAS nations]]
- [[2025-11-04 The Double Standard Why Are Internet Governance Rules Different for Africa]]
- [[2026-07-13 Djibouti Seeks ITU, Smart Africa Support for Telecoms and AI Development]]
- [[2024-07-02 Ghana develops its National Data Strategy with Smart Africa and Team Europe]]
- [[2025-11-13 Transform Africa Summit 2025 opens in Conakry, epicentre of digital cooperation in Africa]]
- [[2025-11-19 Transform Africa Summit 2025 (Conakry) - Simandou 2040 Pillar 3, TELEMO, SANIA-SADX and the closing wrap]]
- [[2025-09-09 Get to know The Republic of Guinea Home of Transform Africa Summit 2025]]
- [[2025-07-14 Smart Africa co-creates Africa's first Cross-Border Data Exchange Guidelines (Rabat, ADD-chaired CAITA)]]
- [[2025-11-17-smart-africa-inaugural-africa-ai-council]]
