---
type: source
title: "Madagascar Safety Nets and Resilience Project \u2014 Mobile Money Delivery Component"
url: https://projects.worldbank.org/en/projects-operations/project-detail/P179466
publisher: World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A)
published: 2022-12-21
date_precision: day
date_source: source
places: [MDG]
topics: [dpi.pay, finance.new, dpi.id]
entities: [[world-bank]]
financier_slug: world-bank
lens: []
deal_id: wb-mdg-003
finance_origin: non-state
ingested: 2026-07-21
retrieved: 2026-06-24
---
# Madagascar Safety Nets and Resilience Project — Mobile Money Delivery Component

World Bank Group committed US$25,000,000 to an unspecified recipient (MDG).

## Deal record

| Field | Value |
|---|---|
| Deal ID | wb-mdg-003 |
| Financier | World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A) |
| Beneficiary type | Government |
| Instrument | Concessional loan *(source label, unverified — see Notes)* |
| Status | Active |
| Commitment (USD) | US$25,000,000 |
| Original amount | USD 25,000,000 |
| Commitment year | 2022 |
| Start year | 2022 |
| End year | 2027 |
| Amount quality | Imputed |
| WB DT category | Payment Systems and Digital Finance -> dpi.pay |
| Digital ID flag | TRUE |
| Digital payments flag | TRUE |
| Project ID | P179466 |
| Board approval date | 2022-12-21 *(enriched from source, 2026-07-21)* |
| Closing date | 2027-03-31 *(enriched from source, 2026-07-21)* |
| Borrower | The Republic of Madagascar *(enriched from source, 2026-07-21)* |
| Implementing agency | Fonds d'Intervention pour le D�veloppement (FID), Ministry of Population, Social Protection and Promotion of Women *(enriched from source, 2026-07-21)* |
| Status (WB) | Active *(enriched from source, 2026-07-21)* |
| Lending instrument | Investment Project Financing *(enriched from source, 2026-07-21)* |

## Description

Digital payment delivery sub-component (~$25M imputed of $250M total) of the Madagascar Safety Nets and Resilience Project. Supports mobile money integration for direct cash transfer delivery, beneficiary digital ID registration, and electronic payment systems for social protection.

## Source

https://projects.worldbank.org/en/projects-operations/project-detail/P179466 - Project portal, accessed 2026-06-24

## Development history

*(empty at build. Definite-match reporting appends one dated, attributed line here.)*

## Notes

Sub-component of parent project P179466 (parent total $250,000,000)

Crosswalk confidence: **high**.

Enrichment: wb-api ok.

Flags: published raised to day precision from enriched board-approval date; recip_org blank in source -> single entity tag; recipient unspecified

The capture labelled **74 of its 75 World Bank rows** `Concessional loan` regardless of the actual instrument. Verified against the World Bank portal, seven are IBRD loans and therefore not concessional; the portal's `idacommamt` field does not separate an IDA credit from an IDA grant, so for IDA operations the label is **unverified** — treat it as "IDA commitment", not established as a loan, unless a primary says otherwise.
