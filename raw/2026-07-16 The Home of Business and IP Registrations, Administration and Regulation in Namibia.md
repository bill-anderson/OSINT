---
type: source
title: The Home of Business and IP Registrations, Administration and Regulation in Namibia
url: https://www.bipa.na/
publisher: Business and Intellectual Property Authority (BIPA)
published: 2026-07-16
date_precision: day
date_source: proxy
accessed: 2026-07-16
ingested: 2026-07-16
places: [NAM]
topics: [dpi.registry, dpi.govtech]
entities: [[bipa-namibia]]
lens: []
body_completeness: full
---

Home/profile page of the Business and Intellectual Property Authority (BIPA), Namibia's focal point for the registration of businesses and industrial/intellectual property rights. BIPA is established as a juristic person under section 3 of the BIPA Act, 2016 (Act No. 8 of 2016) and is a Public Enterprise under the Public Enterprises Governance Act, 2019 (Act No. 1 of 2019).

The portal offers online services including business name search, online name reservation and application-status verification, part of BIPA's stated aim to improve service delivery and the administration of business and IP registrations. Captured as an entity-profile source (no dated event); the page carries no publication date, so the date is proxied to the ingest date.
