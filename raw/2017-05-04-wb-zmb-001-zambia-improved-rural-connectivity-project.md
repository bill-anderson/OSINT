---
type: source
title: "Zambia: Improved Rural Connectivity Project"
url: https://projects.worldbank.org/en/projects-operations/project-detail/P159330
publisher: World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A)
published: 2017-05-04
date_precision: day
date_source: source
places: [ZMB]
topics: [infra.connect, finance.new]
entities: [[world-bank], [ministry-of-finance]]
financier_slug: world-bank
recipient_slug: ministry-of-finance
lens: []
deal_id: wb-zmb-001
finance_origin: non-state
ingested: 2026-07-21
retrieved: 2026-06-24
---
# Zambia: Improved Rural Connectivity Project

World Bank Group committed US$200,000,000 to Ministry of Finance (ZMB).

## Deal record

| Field | Value |
|---|---|
| Deal ID | wb-zmb-001 |
| Financier | World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A) |
| Recipient | Ministry of Finance (ZMB) |
| Beneficiary type | Government |
| Instrument | Concessional loan *(source label, unverified — see Notes)* |
| Status | Active |
| Commitment (USD) | US$200,000,000 |
| Original amount | USD 200,000,000 |
| Commitment year | 2017 |
| Start year | 2017 |
| End year | 2026 |
| Disbursed (USD) | US$141,100,654 |
| Amount quality | Exact |
| WB DT category | Broadband Infrastructure & Services -> infra.connect |
| Digital ID flag | FALSE |
| Digital payments flag | FALSE |
| Project ID | P159330 |
| IATI activity ID | 44000-P159330 |
| Board approval date | 2017-05-04 *(enriched from source, 2026-07-21)* |
| Closing date | 2025-12-31 *(enriched from source, 2026-07-21)* |
| Borrower | Ministry of Finance *(enriched from source, 2026-07-21)* |
| Implementing agency | National Road Fund Agency *(enriched from source, 2026-07-21)* |
| Status (WB) | Active *(enriched from source, 2026-07-21)* |
| Lending instrument | Investment Project Financing *(enriched from source, 2026-07-21)* |

## Description

Improves rural connectivity in Zambia through expansion of broadband infrastructure to underserved rural areas, including last-mile connectivity, digital financial services, and e-government access in rural communities.

## Source

https://projects.worldbank.org/en/projects-operations/project-detail/P159330 - Project portal, accessed 2026-06-24

## Development history

*(empty at build. Definite-match reporting appends one dated, attributed line here.)*

## Notes

Crosswalk confidence: **high**.

Enrichment: wb-api ok.

Flags: published raised to day precision from enriched board-approval date

The capture labelled **74 of its 75 World Bank rows** `Concessional loan` regardless of the actual instrument. Verified against the World Bank portal, seven are IBRD loans and therefore not concessional; the portal's `idacommamt` field does not separate an IDA credit from an IDA grant, so for IDA operations the label is **unverified** — treat it as "IDA commitment", not established as a loan, unless a primary says otherwise.
