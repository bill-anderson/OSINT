---
type: source
title: "Niger: Smart Villages for rural growth and digital inclusion"
url: https://projects.worldbank.org/en/projects-operations/project-detail/P167543
publisher: World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A)
published: 2020-07-08
date_precision: day
date_source: source
places: [NER]
topics: [include.access, finance.new]
entities: [[world-bank], [ministere-des-finances]]
financier_slug: world-bank
recipient_slug: ministere-des-finances
lens: []
deal_id: wb-ner-001
finance_origin: non-state
ingested: 2026-07-21
retrieved: 2026-06-24
---
# Niger: Smart Villages for rural growth and digital inclusion

World Bank Group committed US$100,000,000 to Ministere des Finances (NER).

## Deal record

| Field | Value |
|---|---|
| Deal ID | wb-ner-001 |
| Financier | World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A) |
| Recipient | Ministere des Finances (NER) |
| Beneficiary type | Government |
| Instrument | Concessional loan *(source label, unverified — see Notes)* |
| Status | Active |
| Commitment (USD) | US$100,000,000 |
| Original amount | USD 100,000,000 |
| Commitment year | 2020 |
| Start year | 2020 |
| End year | 2026 |
| Disbursed (USD) | US$77,197,698 |
| Amount quality | Exact |
| WB DT category | DIGITAL INCLUSION -> include.access |
| Digital ID flag | FALSE |
| Digital payments flag | FALSE |
| Project ID | P167543 |
| IATI activity ID | 44000-P167543 |
| Board approval date | 2020-07-08 *(enriched from source, 2026-07-21)* |
| Closing date | 2026-10-23 *(enriched from source, 2026-07-21)* |
| Borrower | Ministere des Finances *(enriched from source, 2026-07-21)* |
| Implementing agency | Projet Villages Intelligents, Unite de Gestion du Projet, ANSI -- Agence Nationale pour la Soci�t� de l'Information / Na *(enriched from source, 2026-07-21)* |
| Status (WB) | Active *(enriched from source, 2026-07-21)* |
| Lending instrument | Investment Project Financing *(enriched from source, 2026-07-21)* |

## Description

Supports digital inclusion and rural development in Niger through smart village initiatives, broadband connectivity, digital financial services, and digital skills.

## Source

https://projects.worldbank.org/en/projects-operations/project-detail/P167543 - Project portal, accessed 2026-06-24

## Development history

*(empty at build. Definite-match reporting appends one dated, attributed line here.)*

## Notes

Crosswalk confidence: **med**.

Enrichment: wb-api ok.

Flags: published raised to day precision from enriched board-approval date

The capture labelled **74 of its 75 World Bank rows** `Concessional loan` regardless of the actual instrument. Verified against the World Bank portal, seven are IBRD loans and therefore not concessional; the portal's `idacommamt` field does not separate an IDA credit from an IDA grant, so for IDA operations the label is **unverified** — treat it as "IDA commitment", not established as a loan, unless a primary says otherwise.

**Enriched field truncated at build.** The load cut enriched values at 120 characters, so this record's `Implementing agency` / `Borrower` value is incomplete. Re-read it from the World Bank portal before relying on it.
