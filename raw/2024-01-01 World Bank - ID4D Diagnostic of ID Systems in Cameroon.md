---
type: source
title: "Identification for Development (ID4D): Diagnostic of ID Systems in Cameroon"
publisher: World Bank
url: https://documents.worldbank.org/en/publication/documents-reports
author: World Bank (ID4D)
published: 2024-01-01
date_precision: year
date_source: proxy
accessed: 2026-07-15
ingested: 2026-07-15
places: [CMR]
topics: [dpi.id, dpi.registry, include.access]
entities: [[world-bank], [bunec]]
lens: []
---

<!-- COMPANION SOURCE PAGE for binary raw/2024-01-01 World Bank - ID4D Diagnostic of ID Systems in Cameroon.pdf (103pp; date proxied to year). World Bank ID4D diagnostic of Cameroon's identification + civil-registration systems — the authoritative baseline behind the 2024-25 reforms (new biometric CNI via Augentic/DGSN, BUNEC CRVS digitisation, UPIN, Law 2024/016). Per prior guidance, held as a reference; specific figures not promoted to synthesis pages. -->

**Binary artefact:** [[2024-01-01 World Bank - ID4D Diagnostic of ID Systems in Cameroon.pdf]] (103 pages).

World Bank ID4D diagnostic of **Cameroon's identification and civil-registration systems** — the authoritative baseline behind the 2024–25 reforms (the new **[[augentic]]/[[dgsn-cameroon]]** biometric CNI, **[[bunec]]** CRVS digitisation, the UPIN and Law 2024/016). Held as a reference; per-figure detail not lifted into synthesis pages. See [[cameroon--dpi-id]], [[dpi.id]].
