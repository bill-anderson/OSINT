---
type: source
title: "Senegal Digital Economy Acceleration Project"
url: https://projects.worldbank.org/en/projects-operations/project-detail/P172524
publisher: World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A)
published: 2023-02-09
date_precision: day
date_source: source
places: [SEN]
topics: [infra.connect, finance.new]
entities: [[world-bank], [republic-of-senegal]]
lens: []
deal_id: wb-sen-001
finance_origin: non-state
ingested: 2026-07-21
retrieved: 2026-06-24
---
# Senegal Digital Economy Acceleration Project

World Bank Group committed US$150,000,000 to Republic of Senegal (SEN).

## Deal record

| Field | Value |
|---|---|
| Deal ID | wb-sen-001 |
| Financier | World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A) |
| Recipient | Republic of Senegal (SEN) |
| Beneficiary type | Government |
| Instrument | Concessional loan *(source label, unverified — see Notes)* |
| Status | Active |
| Commitment (USD) | US$150,000,000 |
| Original amount | USD 150,000,000 |
| Commitment year | 2023 |
| Start year | 2023 |
| End year | 2028 |
| Disbursed (USD) | US$8,883,367 |
| Amount quality | Exact |
| WB DT category | DIGITAL ECONOMY -> infra.connect |
| Digital ID flag | FALSE |
| Digital payments flag | FALSE |
| Project ID | P172524 |
| IATI activity ID | 44000-P172524 |
| Board approval date | 2023-02-09 *(enriched from source, 2026-07-21)* |
| Closing date | 2028-05-31 *(enriched from source, 2026-07-21)* |
| Borrower | Republic of Senegal *(enriched from source, 2026-07-21)* |
| Implementing agency | Ministry of Health and Social Action (MSAS), Ministry of Communication, Telecommunications, and Digital Economy (MCTEN) *(enriched from source, 2026-07-21)* |
| Status (WB) | Active *(enriched from source, 2026-07-21)* |
| Lending instrument | Investment Project Financing *(enriched from source, 2026-07-21)* |

## Description

Supports Senegal's digital economy through improved connectivity, digital public infrastructure, digital financial services, and digital skills. Part of DE4A initiative.

## Source

https://projects.worldbank.org/en/projects-operations/project-detail/P172524 - Project portal, accessed 2026-06-24

## Development history

*(empty at build. Definite-match reporting appends one dated, attributed line here.)*

## Notes

Subject slug inferred from "connectivity".

Crosswalk confidence: **review**.

Enrichment: wb-api ok.

Flags: slug inferred from "connectivity"; crosswalk confidence=review; published raised to day precision from enriched board-approval date

The capture labelled **74 of its 75 World Bank rows** `Concessional loan` regardless of the actual instrument. Verified against the World Bank portal, seven are IBRD loans and therefore not concessional; the portal's `idacommamt` field does not separate an IDA credit from an IDA grant, so for IDA operations the label is **unverified** — treat it as "IDA commitment", not established as a loan, unless a primary says otherwise.

**Enriched field truncated at build.** The load cut enriched values at 120 characters, so this record's `Implementing agency` / `Borrower` value is incomplete. Re-read it from the World Bank portal before relying on it.
