---
type: source
title: "Digital Governance and Identification Management System Project"
url: https://projects.worldbank.org/en/projects-operations/project-detail/P169413
publisher: World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A)
published: 2020-09-29
date_precision: day
date_source: source
places: [MDG]
topics: [dpi.id, finance.new]
entities: [[world-bank], [government-of-madagascar]]
lens: []
deal_id: wb-mdg-002
finance_origin: non-state
ingested: 2026-07-21
retrieved: 2026-06-24
last_reviewed: 2026-07-22
---
# Digital Governance and Identification Management System Project

World Bank Group committed US$143,000,000 to Government of Madagascar (MDG).

## Deal record

| Field | Value |
|---|---|
| Deal ID | wb-mdg-002 |
| Financier | World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A) |
| Recipient | Government of Madagascar (MDG) |
| Beneficiary type | Government |
| Instrument | Concessional loan *(source label, unverified — see Notes)* |
| Status | Active |
| Commitment (USD) | US$143,000,000 |
| Original amount | USD 143,000,000 |
| Commitment year | 2020 |
| Start year | 2020 |
| End year | 2026 |
| Amount quality | Exact |
| WB DT category | Identification Systems and Civil Registration -> dpi.id |
| Digital ID flag | TRUE |
| Digital payments flag | FALSE |
| Project ID | P169413 |
| IATI activity ID | 44000-P169413 |
| Board approval date | 2020-09-29 *(enriched from source, 2026-07-21)* |
| Closing date | 2026-06-30 *(enriched from source, 2026-07-21)* |
| Borrower | Government of Madagascar *(enriched from source, 2026-07-21)* |
| Implementing agency | PRODIGY PIU *(enriched from source, 2026-07-21)* |
| Status (WB) | Active *(enriched from source, 2026-07-21)* |
| Lending instrument | Investment Project Financing *(enriched from source, 2026-07-21)* |

## Description

Supports Madagascar's digital governance and national ID system through digital identification infrastructure, e-government platforms, and digital financial services.

## Source

https://projects.worldbank.org/en/projects-operations/project-detail/P169413 - Project portal, accessed 2026-06-24

## Development history

- **2026-06-24** — PRODIGY (US$143m) entered its closing stage: two renewable-powered data centres (Tsimbazaza, Iavoloha) under construction to host and secure the national digital-identity system. [madagascar-worldbank-review-543m]

## Notes

Crosswalk confidence: **high**.

Enrichment: wb-api ok.

Flags: published raised to day precision from enriched board-approval date

The capture labelled **74 of its 75 World Bank rows** `Concessional loan` regardless of the actual instrument. Verified against the World Bank portal, seven are IBRD loans and therefore not concessional; the portal's `idacommamt` field does not separate an IDA credit from an IDA grant, so for IDA operations the label is **unverified** — treat it as "IDA commitment", not established as a loan, unless a primary says otherwise.
