---
type: source
title: "Cameroon Adaptive Safety Nets and Economic Inclusion \u2014 Digital Cash Transfer Delivery"
url: https://projects.worldbank.org/en/projects-operations/project-detail/P175363
publisher: World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A)
published: 2022-03-09
date_precision: day
date_source: source
places: [CMR]
topics: [dpi.pay, finance.new]
entities: [[world-bank]]
financier_slug: world-bank
lens: []
deal_id: wb-cmr-003
finance_origin: non-state
ingested: 2026-07-21
retrieved: 2026-06-24
---
# Cameroon Adaptive Safety Nets and Economic Inclusion — Digital Cash Transfer Delivery

World Bank Group committed US$16,000,000 to an unspecified recipient (CMR).

## Deal record

| Field | Value |
|---|---|
| Deal ID | wb-cmr-003 |
| Financier | World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A) |
| Beneficiary type | Government |
| Instrument | Concessional loan *(source label, unverified — see Notes)* |
| Status | Active |
| Commitment (USD) | US$16,000,000 |
| Original amount | USD 16,000,000 |
| Commitment year | 2022 |
| Start year | 2022 |
| End year | 2028 |
| Amount quality | Imputed |
| WB DT category | Payment Systems and Digital Finance -> dpi.pay |
| Digital ID flag | FALSE |
| Digital payments flag | TRUE |
| Project ID | P175363 |
| Board approval date | 2022-03-09 *(enriched from source, 2026-07-21)* |
| Closing date | 2028-03-30 *(enriched from source, 2026-07-21)* |
| Borrower | The Republic of Cameroon *(enriched from source, 2026-07-21)* |
| Implementing agency | Ministry of Economy, Planning, and Regional Development (MINEPAT) *(enriched from source, 2026-07-21)* |
| Status (WB) | Active *(enriched from source, 2026-07-21)* |
| Lending instrument | Investment Project Financing *(enriched from source, 2026-07-21)* |

## Description

Digital delivery system sub-component (~$16M imputed of $160M total) of the Cameroon Adaptive Safety Nets and Economic Inclusion Project. Supports digital payment infrastructure for cash transfers, beneficiary management information systems, and mobile money integration for social transfers.

## Source

https://projects.worldbank.org/en/projects-operations/project-detail/P175363 - Project portal, accessed 2026-06-24

## Development history

*(empty at build. Definite-match reporting appends one dated, attributed line here.)*

## Notes

Sub-component of parent project P175363 (parent total $160,000,000)

Crosswalk confidence: **high**.

Enrichment: wb-api ok.

Flags: published raised to day precision from enriched board-approval date; recip_org blank in source -> single entity tag; recipient unspecified

The capture labelled **74 of its 75 World Bank rows** `Concessional loan` regardless of the actual instrument. Verified against the World Bank portal, seven are IBRD loans and therefore not concessional; the portal's `idacommamt` field does not separate an IDA credit from an IDA grant, so for IDA operations the label is **unverified** — treat it as "IDA commitment", not established as a loan, unless a primary says otherwise.
