---
type: source
title: "Sustainable Energy and Broadband Access in Rural Mozambique Project"
url: https://projects.worldbank.org/en/projects-operations/project-detail/P175295
publisher: World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A)
published: 2021-12-14
date_precision: day
date_source: source
places: [MOZ]
topics: [infra.connect, finance.new]
entities: [[world-bank], [republic-of-mozambique]]
lens: []
deal_id: wb-moz-002
finance_origin: non-state
ingested: 2026-07-21
retrieved: 2026-06-24
---
# Sustainable Energy and Broadband Access in Rural Mozambique Project

World Bank Group committed US$343,000,000 to Republic of Mozambique (MOZ).

## Deal record

| Field | Value |
|---|---|
| Deal ID | wb-moz-002 |
| Financier | World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A) |
| Recipient | Republic of Mozambique (MOZ) |
| Beneficiary type | Government |
| Instrument | Concessional loan *(source label, unverified — see Notes)* |
| Status | Active |
| Commitment (USD) | US$343,000,000 |
| Original amount | USD 343,000,000 |
| Commitment year | 2021 |
| Start year | 2021 |
| End year | 2027 |
| Disbursed (USD) | US$190,819,278 |
| Amount quality | Rounded |
| WB DT category | Broadband Infrastructure & Services -> infra.connect |
| Digital ID flag | FALSE |
| Digital payments flag | FALSE |
| Project ID | P175295 |
| IATI activity ID | 44000-P175295 |
| Board approval date | 2021-12-14 *(enriched from source, 2026-07-21)* |
| Closing date | 2027-11-30 *(enriched from source, 2026-07-21)* |
| Borrower | Republic of Mozambique *(enriched from source, 2026-07-21)* |
| Implementing agency | Fundo de Energia (FUNAE), Ministry of Mineral Resources and Energy (MIREME), Electricidade de Mo�ambique (EdM) *(enriched from source, 2026-07-21)* |
| Status (WB) | Active *(enriched from source, 2026-07-21)* |
| Lending instrument | Investment Project Financing *(enriched from source, 2026-07-21)* |

## Description

Expands rural broadband access and provides sustainable energy in Mozambique through shared infrastructure investment. Supports last-mile broadband connectivity alongside rural electrification.

## Source

https://projects.worldbank.org/en/projects-operations/project-detail/P175295 - Project portal, accessed 2026-06-24

## Development history

*(empty at build. Definite-match reporting appends one dated, attributed line here.)*

## Notes

Dual-purpose: Energy and Broadband Access. Digital (broadband) component is a key element. Total project: $343M.

Crosswalk confidence: **high**.

Enrichment: wb-api ok.

Flags: published raised to day precision from enriched board-approval date

The capture labelled **74 of its 75 World Bank rows** `Concessional loan` regardless of the actual instrument. Verified against the World Bank portal, seven are IBRD loans and therefore not concessional; the portal's `idacommamt` field does not separate an IDA credit from an IDA grant, so for IDA operations the label is **unverified** — treat it as "IDA commitment", not established as a loan, unless a primary says otherwise.
