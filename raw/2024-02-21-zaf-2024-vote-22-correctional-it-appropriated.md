---
type: source
title: "South Africa Vote 22 (Correctional Services) — Information Technology subprogramme, FY2024/25 appropriated: R358.0m"
url: https://www.treasury.gov.za/documents/national%20budget/2024/ene/FullENE.pdf
publisher: National Treasury (South Africa)
published: 2024-02-21
date_precision: day
date_source: source
topics: [dpi.govtech, finance.budget]
entities: [[department-of-correctional-services-south-africa]]
financier_slug: department-of-correctional-services-south-africa
deal_id: zaf-2024-vote-22-correctional-it-appropriated
amount_total: 358000000
places: [ZAF]
lens: []
finance_origin: domestic-state
state_level: national
spending_tier_name: ""
fiscal_year_label: "2024/25"
fy_start: 2024-04-01
fy_end: 2025-03-31
budget_stage: appropriated
budget_version: original
source_tier: budget-document
supplementary_basis: ""
scope_confidence: whole
is_transfer: false
currency: ZAR
ingested: 2026-07-22
retrieved: 2026-07-22
body_completeness: full
---

# South Africa Vote 22 (Correctional Services) — Information Technology subprogramme, FY2024/25 appropriated: R358.0m

## Deal record

| Field | Value |
|---|---|
| Deal ID | zaf-2024-vote-22-correctional-it-appropriated |
| Financier | National fisc — vote appropriation |
| Vote | 22 Correctional Services |
| Subprogramme | Information Technology (Administration) |
| Appropriated | R358,000,000 |
| doc_type | budget-estimates |
| doc_locator | ENE 2024 full volume (FullENE.pdf) |
| amount_scale | R million as printed — stored in units |
| fy_calendar | gregorian |
| funding_source | unstated (National Revenue Fund appropriation) |

## Description

ENE 2024, Vote 22 subprogramme table. Series: 205.6 / 301.9 / 297.1 / 370.6 (2023/24 adj) -> 358.0 / 376.3 / 393.6 (MTEF).

## Development history

## Notes

Departmental IT subprogramme — recorded per the uniform grain rule (an identifiable IT/digital subprogramme line is capturable; see extraction notes). *Cross-vote scan record (BUDGET-EXTRACT step 4a, 2026-07-22): outturn for this vote's lines is not held — the vote's own annual report is the acquirable outturn instrument.*
