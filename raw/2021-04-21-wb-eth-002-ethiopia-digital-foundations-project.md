---
type: source
title: "Ethiopia Digital Foundations Project"
url: https://projects.worldbank.org/en/projects-operations/project-detail/P171034
publisher: World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A)
published: 2021-04-21
date_precision: day
date_source: source
places: [ETH]
topics: [infra.connect, finance.new]
entities: [[world-bank], [federal-democratic-republic-of-ethiopia]]
lens: []
deal_id: wb-eth-002
finance_origin: non-state
ingested: 2026-07-21
retrieved: 2026-06-24
---
# Ethiopia Digital Foundations Project

World Bank Group committed US$200,000,000 to Federal Democratic Republic of Ethiopia (ETH).

## Deal record

| Field | Value |
|---|---|
| Deal ID | wb-eth-002 |
| Financier | World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A) |
| Recipient | Federal Democratic Republic of Ethiopia (ETH) |
| Beneficiary type | Government |
| Instrument | Concessional loan *(source label, unverified — see Notes)* |
| Status | Active |
| Commitment (USD) | US$200,000,000 |
| Original amount | USD 200,000,000 |
| Commitment year | 2021 |
| Start year | 2021 |
| End year | 2026 |
| Disbursed (USD) | US$138,718,955 |
| Amount quality | Exact |
| WB DT category | DIGITAL ECONOMY -> infra.connect |
| Digital ID flag | FALSE |
| Digital payments flag | FALSE |
| Project ID | P171034 |
| IATI activity ID | 44000-P171034 |
| Board approval date | 2021-04-21 *(enriched from source, 2026-07-21)* |
| Closing date | 2026-10-31 *(enriched from source, 2026-07-21)* |
| Borrower | Federal Democratic Republic of Ethiopia *(enriched from source, 2026-07-21)* |
| Implementing agency | Ministry of Finance and Economic Development, Ministry of Innovation and Technology (MInT) *(enriched from source, 2026-07-21)* |
| Status (WB) | Active *(enriched from source, 2026-07-21)* |
| Lending instrument | Investment Project Financing *(enriched from source, 2026-07-21)* |

## Description

Establishes digital foundations in Ethiopia including broadband infrastructure, digital government platforms, cybersecurity frameworks, and digital skills.

## Source

https://projects.worldbank.org/en/projects-operations/project-detail/P171034 - Project portal, accessed 2026-06-24

## Development history

*(empty at build. Definite-match reporting appends one dated, attributed line here.)*

## Notes

Subject slug inferred from "broadband".

Crosswalk confidence: **review**.

Enrichment: wb-api ok.

Flags: slug inferred from "broadband"; crosswalk confidence=review; published raised to day precision from enriched board-approval date

The capture labelled **74 of its 75 World Bank rows** `Concessional loan` regardless of the actual instrument. Verified against the World Bank portal, seven are IBRD loans and therefore not concessional; the portal's `idacommamt` field does not separate an IDA credit from an IDA grant, so for IDA operations the label is **unverified** — treat it as "IDA commitment", not established as a loan, unless a primary says otherwise.
