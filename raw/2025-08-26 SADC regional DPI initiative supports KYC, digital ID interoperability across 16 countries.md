---
type: source
title: "Regional DPI initiative supports KYC, digital ID interoperability across 16 countries"
publisher: Biometric Update
url: https://www.biometricupdate.com/202508/regional-dpi-initiative-supports-kyc-digital-id-interoperability-across-16-countries
author: Ayang Macdonald
published: 2025-08-26
date_precision: day
date_source: source
accessed: 2026-07-15
ingested: 2026-07-15
places: [XSA, BWA]
topics: [dpi.id, dpi.exchange, dpi.pay, gov.regional]
entities: [[sadc], [sadc-cross-border-dpi-framework]]
lens: [sovereignty]
body_completeness: full
---

<!-- Biometric Update (Aug 2025): "Africa's first" regional cross-border DPI/digital-ID framework unveiled — a partnership of the SADC Committee of Central Bank Governors (CCBG), Co-Develop (DPI fund; 50-in-5 campaign partner) and FinMark Trust (financial-inclusion advocacy). Aim: a federated digital-eKYC ecosystem across the 16 SADC member states (Angola, Botswana, Comoros, DRC, Eswatini, Lesotho, Madagascar, Malawi, Mauritius, Mozambique, Namibia, Seychelles, South Africa, Tanzania, Zambia, Zimbabwe). FinMark Trust provides technical support for federated digital ID + interoperable data exchange + enhanced digital payments. Follows a real-time cross-border ID-verification pilot between South Africa and Lesotho. Problem: siloed national IDs block >3.7m nationals living outside their home countries from banking/payments; a shared reusable rail replaces 16 bilateral agreements. Co-Develop's Robert Karanja; FinMark Trust CEO Brendan Pearce. Regional (XSA); tags BWA as a member touched by this batch. -->

A partnership unveiled (August 2025) as **"Africa's first" regional cross-border digital-ID framework** — the **SADC Committee of Central Bank Governors (CCBG)**, **Co-Develop** (a DPI fund and 50-in-5 campaign partner) and **FinMark Trust** — aims to build a **federated digital eKYC ecosystem across the 16 SADC member states** (Angola, Botswana, Comoros, DRC, Eswatini, Lesotho, Madagascar, Malawi, Mauritius, Mozambique, Namibia, Seychelles, South Africa, Tanzania, Zambia, Zimbabwe).

FinMark Trust provides technical support for the integrated components — **federated digital ID, interoperable data exchange and enhanced digital payments** — following a real-time cross-border ID-verification-for-payments pilot between **South Africa and Lesotho**. The problem it targets: siloed national ID systems block **more than 3.7 million** nationals living outside their home countries from banking and digital payments; a shared, reusable rail replaces the need for 16 bilateral agreements. Co-Develop's Robert Karanja called SADC's regional-DPI leadership a model for "transformation at unprecedented scale."
