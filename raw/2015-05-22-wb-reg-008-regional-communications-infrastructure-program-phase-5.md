---
type: source
title: "Regional Communications Infrastructure Program Phase 5 - Uganda"
url: https://projects.worldbank.org/en/projects-operations/project-detail/P130871
publisher: World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A)
published: 2015-05-22
date_precision: day
date_source: source
places: [XEA]
topics: [infra.connect, finance.new]
entities: [[world-bank], [republic-of-uganda]]
financier_slug: world-bank
recipient_slug: republic-of-uganda
lens: []
deal_id: wb-reg-008
finance_origin: non-state
ingested: 2026-07-21
retrieved: 2026-06-24
---
# Regional Communications Infrastructure Program Phase 5 - Uganda

World Bank Group committed US$75,000,000 to Republic of Uganda (XEA).

## Deal record

| Field | Value |
|---|---|
| Deal ID | wb-reg-008 |
| Financier | World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A) |
| Recipient | Republic of Uganda (XEA) |
| Beneficiary type | Government |
| Instrument | Concessional loan *(source label, unverified — see Notes)* |
| Status | Closed |
| Commitment (USD) | US$75,000,000 |
| Original amount | USD 75,000,000 |
| Commitment year | 2015 |
| Start year | 2015 |
| End year | 2022 |
| Disbursed (USD) | US$75,515,232 |
| Amount quality | Exact |
| WB DT category | Broadband Infrastructure & Services -> infra.connect |
| Digital ID flag | FALSE |
| Digital payments flag | FALSE |
| Project ID | P130871 |
| IATI activity ID | 44000-P130871 |
| Board approval date | 2015-05-22 *(enriched from source, 2026-07-21)* |
| Closing date | 2022-08-31 *(enriched from source, 2026-07-21)* |
| Borrower | Republic of Uganda *(enriched from source, 2026-07-21)* |
| Implementing agency | National Information Technology Authority, Uganda, Ministry of ICT and National Guidance *(enriched from source, 2026-07-21)* |
| Status (WB) | Closed *(enriched from source, 2026-07-21)* |
| Lending instrument | Investment Project Financing *(enriched from source, 2026-07-21)* |

## Description

Extends RCIP regional communications infrastructure program to improve broadband connectivity in Uganda and the East Africa subregion.

## Source

https://projects.worldbank.org/en/projects-operations/project-detail/P130871 - Project portal, accessed 2026-06-24

## Development history

*(empty at build. Definite-match reporting appends one dated, attributed line here.)*

## Notes

Crosswalk confidence: **high**.

Enrichment: wb-api ok.

Flags: published raised to day precision from enriched board-approval date

The capture labelled **74 of its 75 World Bank rows** `Concessional loan` regardless of the actual instrument. Verified against the World Bank portal, seven are IBRD loans and therefore not concessional; the portal's `idacommamt` field does not separate an IDA credit from an IDA grant, so for IDA operations the label is **unverified** — treat it as "IDA commitment", not established as a loan, unless a primary says otherwise.
