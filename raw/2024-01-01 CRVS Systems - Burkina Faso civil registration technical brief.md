---
type: source
title: "CRVS Systems — Burkina Faso civil registration (technical brief / webinar)"
publisher: Centre of Excellence for CRVS Systems (IDRC)
url: https://crvssystems.ca/
author: Centre of Excellence for CRVS Systems
published: 2024-01-01
date_precision: year
date_source: proxy
accessed: 2026-07-15
ingested: 2026-07-15
places: [BFA]
topics: [dpi.id, dpi.registry]
entities: [[veneem]]
lens: []
body_completeness: full
---

<!-- COMPANION SOURCE PAGE for binary raw/2024-01-01 CRVS Systems - Burkina Faso civil registration technical brief.pdf (10pp; date proxied to year). Technical brief on Burkina Faso's civil registration, incl. continuity during the COVID-19 crisis (civil registration declared an essential service from 9 Mar 2020, staff rotation under lockdown). Reference context for the CRVS foundation ([[veneem]], unique identifier) beneath the ID stack. -->

**Binary artefact:** [[2024-01-01 CRVS Systems - Burkina Faso civil registration technical brief.pdf]] (10 pages).

Centre of Excellence for CRVS Systems technical brief on Burkina Faso's civil registration — including how it maintained continuity through the COVID-19 crisis (civil registration declared an essential service from March 2020, with staff rotation under lockdown). Reference background for the CRVS foundation ([[veneem]], the unique identifier) under Burkina's ID stack. See [[dpi.registry]].
