---
type: source
title: "DGSN Institutional Brochure (Morocco)"
url: https://www.dgsn.gov.ma
url_note: "url unrecovered as of 2026-07-21 (lint #14). dgsn.gov.ma refuses connections from outside Morocco and the Wayback CDX index holds no PDF captures for the host, so no document path can be established."
publisher: DGSN (Direction Générale de la Sûreté Nationale, Morocco)
author: DGSN
published: 2024-09-18
date_precision: day
date_source: source
ingested: 2026-07-16
places: [MAR]
topics: [dpi.id, gov.policy]
entities: [[dgsn-morocco]]
lens: []
body_completeness: full
artefact: "2024-09-18 PlaquetteInstitutionnel DGSN Morocco (Ang).pdf"
---

# DGSN Institutional Brochure (Morocco)

The institutional brochure (Plaquette Institutionnelle, dated 18 September 2024, English edition) of
Morocco's **[[dgsn-morocco|DGSN]]** (General Directorate of National Security) — the body that issues the
electronic national ID and runs the national digital-identity / trusted-third-party verification
platform ([[dpi.id]]). Establishing/reference material for the DGSN entity. See [[MAR]].

**Primary document (PDF):** `2024-09-18 PlaquetteInstitutionnel DGSN Morocco (Ang).pdf` — held in `raw/`. See [[dgsn-morocco]].
