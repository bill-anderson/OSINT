---
type: source
title: "Cote d'Ivoire Youth Employment and Skills Development Phase 2 \u2014 Digital Skills Component"
url: https://projects.worldbank.org/en/projects-operations/project-detail/P172800
publisher: World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A)
published: 2022-01-31
date_precision: day
date_source: source
places: [CIV]
topics: [capacity.training, finance.new]
entities: [[world-bank]]
lens: []
deal_id: wb-civ-003
finance_origin: non-state
ingested: 2026-07-21
retrieved: 2026-06-24
---
# Cote d'Ivoire Youth Employment and Skills Development Phase 2 — Digital Skills Component

World Bank Group committed US$20,000,000 to an unspecified recipient (CIV).

## Deal record

| Field | Value |
|---|---|
| Deal ID | wb-civ-003 |
| Financier | World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A) |
| Beneficiary type | Government |
| Instrument | Concessional loan *(source label, unverified — see Notes)* |
| Status | Active |
| Commitment (USD) | US$20,000,000 |
| Original amount | USD 20,000,000 |
| Commitment year | 2022 |
| Start year | 2022 |
| End year | 2026 |
| Amount quality | Imputed |
| WB DT category | DIGITAL SKILLS AND JOBS -> capacity.training |
| Digital ID flag | FALSE |
| Digital payments flag | FALSE |
| Project ID | P172800 |
| Board approval date | 2022-01-31 *(enriched from source, 2026-07-21)* |
| Closing date | 2026-12-31 *(enriched from source, 2026-07-21)* |
| Borrower | Republic of Cote d'Ivoire *(enriched from source, 2026-07-21)* |
| Implementing agency | Bureau de Coordination des Programmes Emploi, Ministry of Technical and Vocational Training and Apprenticeship *(enriched from source, 2026-07-21)* |
| Status (WB) | Active *(enriched from source, 2026-07-21)* |
| Lending instrument | Investment Project Financing *(enriched from source, 2026-07-21)* |

## Description

Digital skills and job matching sub-component (~$20M imputed of $150M total) of the Cote d'Ivoire Youth Employment and Skills Development Project Phase 2. Includes digital literacy training, ICT skills development, and digital job matching platform for youth employment in the digital economy.

## Source

https://projects.worldbank.org/en/projects-operations/project-detail/P172800 - Project portal, accessed 2026-06-24

## Development history

*(empty at build. Definite-match reporting appends one dated, attributed line here.)*

## Notes

Sub-component of parent project P172800 (parent total $150,000,000)

Crosswalk confidence: **high**.

Enrichment: wb-api ok.

Flags: published raised to day precision from enriched board-approval date; recip_org blank in source -> single entity tag; recipient unspecified

The capture labelled **74 of its 75 World Bank rows** `Concessional loan` regardless of the actual instrument. Verified against the World Bank portal, seven are IBRD loans and therefore not concessional; the portal's `idacommamt` field does not separate an IDA credit from an IDA grant, so for IDA operations the label is **unverified** — treat it as "IDA commitment", not established as a loan, unless a primary says otherwise.
