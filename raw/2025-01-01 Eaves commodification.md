---
type: source
title: "Parting Clouds: Creating a Competitive Marketplace for Compute"
publisher: Canadian Anti-Monopoly Project (CAMP)
url: https://www.antimonopoly.ca/
author: David Eaves, Keldon Bester
published: 2025
date_precision: year
accessed: 2026-07-10
ingested: 2026-07-10
attachment: 2025-01-01 Eaves commodification.pdf
places: [XGL]
topics: [tech.industry, infra.store, gov.standards, gov.legislate, geopol.usa, gov.policy]
entities: [[david-eaves], [camp]]
lens: [sovereignty]
---

<!-- SOURCE RECORD for the PDF `2025-01-01 Eaves commodification.pdf` (source of record). CAMP report,
     2025, co-authored with Keldon Bester (CAMP Exec. Director). Admissible primary.
     Canada-focused but generalises → XGL. Resolves the second Eaves item flagged as
     unfiled on [[david-eaves]]. NOT a duplicate of the Tech Policy Press op-ed — a
     distinct, fuller, co-authored competition-policy report on the same theme. -->

# Parting Clouds: Creating a Competitive Marketplace for Compute

**Eaves, D. and Bester, K. (2025).** Published by the Canadian Anti-Monopoly Project
([[camp]]). Attachment: `2025-01-01 Eaves commodification.pdf`.

## Summary (as captured)

Cloud computing is core economic infrastructure controlled by a "remarkably small number
of firms": three hyperscalers — **Amazon (31%), Microsoft (24%), Alphabet (11%)** — hold
**66%** of global public cloud share; in Canada the same three control **85%** (federal
government spent >$156m on cloud in 2022–23, mostly to Microsoft and Amazon).

The report anatomises the costs of concentration — **vendor lock-in / competitor
lock-out, opaque pricing and asymmetric contracts, digital tollbooths (gatekeeping
adjacent markets), capture of innovation, systemic outage risk, and concentration as
geopolitical coercion** — and argues for market-shaping remedies (interoperability,
procurement leverage) over bespoke national "sovereign stack" industrial policy.

## Why it matters here

The fuller, evidenced version of the argument in Eaves' Tech Policy Press op-ed
([[2025-12-15 The Path to a Sovereign Tech Stack is Via a Commodified Tech Stack  TechPolicy.Press]]):
sovereignty via commodification/interoperability, not ownership. See [[tech.industry]],
[[infra.store]], [[gov.legislate]], [[david-eaves]].
