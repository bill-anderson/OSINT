---
type: source
title: "Rwanda Social Protection Transformation Project \u2014 Digital Payment Systems Component"
url: https://projects.worldbank.org/en/projects-operations/project-detail/P177492
publisher: World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A)
published: 2022-09-22
date_precision: day
date_source: source
places: [RWA]
topics: [dpi.pay, finance.new]
entities: [[world-bank]]
financier_slug: world-bank
lens: []
deal_id: wb-rwa-002
finance_origin: non-state
ingested: 2026-07-21
retrieved: 2026-06-24
---
# Rwanda Social Protection Transformation Project — Digital Payment Systems Component

World Bank Group committed US$13,530,000 to an unspecified recipient (RWA).

## Deal record

| Field | Value |
|---|---|
| Deal ID | wb-rwa-002 |
| Financier | World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A) |
| Beneficiary type | Government |
| Instrument | Concessional loan *(source label, unverified — see Notes)* |
| Status | Active |
| Commitment (USD) | US$13,530,000 |
| Original amount | USD 13,530,000 |
| Commitment year | 2022 |
| Start year | 2022 |
| End year | 2028 |
| Amount quality | Imputed |
| WB DT category | Payment Systems and Digital Finance -> dpi.pay |
| Digital ID flag | FALSE |
| Digital payments flag | TRUE |
| Project ID | P177492 |
| Board approval date | 2022-09-22 *(enriched from source, 2026-07-21)* |
| Closing date | 2026-12-31 *(enriched from source, 2026-07-21)* |
| Borrower | Republic of Rwanda *(enriched from source, 2026-07-21)* |
| Implementing agency | Local Administrative Entities Development Agency (LODA), The Ministry of Local Government (MINALOC) *(enriched from source, 2026-07-21)* |
| Status (WB) | Active *(enriched from source, 2026-07-21)* |
| Lending instrument | Investment Project Financing *(enriched from source, 2026-07-21)* |

## Description

Components 3 and 4 (combined ~$13.5M of $100M total) of the Rwanda Social Protection Transformation Project support nationwide digital payment systems for VUP social transfers via mobile money (MTN/Airtel), SACCO digitization, dynamic social registry, and digital beneficiary management systems.

## Source

https://projects.worldbank.org/en/projects-operations/project-detail/P177492 - Project portal, accessed 2026-06-24

## Development history

*(empty at build. Definite-match reporting appends one dated, attributed line here.)*

## Notes

Sub-component of parent project P177492 (parent total $100,000,000)

Crosswalk confidence: **high**.

Enrichment: wb-api ok.

Flags: published raised to day precision from enriched board-approval date; recip_org blank in source -> single entity tag; recipient unspecified

The capture labelled **74 of its 75 World Bank rows** `Concessional loan` regardless of the actual instrument. Verified against the World Bank portal, seven are IBRD loans and therefore not concessional; the portal's `idacommamt` field does not separate an IDA credit from an IDA grant, so for IDA operations the label is **unverified** — treat it as "IDA commitment", not established as a loan, unless a primary says otherwise.
