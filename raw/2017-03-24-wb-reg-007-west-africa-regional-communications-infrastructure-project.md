---
type: source
title: "West Africa Regional Communications Infrastructure Project - SOP3"
url: https://projects.worldbank.org/en/projects-operations/project-detail/P155876
publisher: World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A)
published: 2017-03-24
date_precision: day
date_source: source
places: [XWA]
topics: [infra.connect, finance.new]
entities: [[world-bank], [the-republic-of-guinea-bissau]]
financier_slug: world-bank
recipient_slug: the-republic-of-guinea-bissau
lens: []
deal_id: wb-reg-007
finance_origin: non-state
ingested: 2026-07-21
retrieved: 2026-06-24
---
# West Africa Regional Communications Infrastructure Project - SOP3

World Bank Group committed US$35,000,000 to The Republic of Guinea Bissau (XWA).

## Deal record

| Field | Value |
|---|---|
| Deal ID | wb-reg-007 |
| Financier | World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A) |
| Recipient | The Republic of Guinea Bissau (XWA) |
| Beneficiary type | Government |
| Instrument | Concessional loan *(source label, unverified — see Notes)* |
| Status | Closed |
| Commitment (USD) | US$35,000,000 |
| Original amount | USD 35,000,000 |
| Commitment year | 2017 |
| Start year | 2017 |
| End year | 2022 |
| Disbursed (USD) | US$34,722,387 |
| Amount quality | Exact |
| WB DT category | Broadband Infrastructure & Services -> infra.connect |
| Digital ID flag | FALSE |
| Digital payments flag | FALSE |
| Project ID | P155876 |
| IATI activity ID | 44000-P155876 |
| Board approval date | 2017-03-24 *(enriched from source, 2026-07-21)* |
| Closing date | 2022-11-30 *(enriched from source, 2026-07-21)* |
| Borrower | The Republic of Guinea Bissau *(enriched from source, 2026-07-21)* |
| Implementing agency | Ministry of Transport and Telecommunications *(enriched from source, 2026-07-21)* |
| Status (WB) | Closed *(enriched from source, 2026-07-21)* |
| Lending instrument | Investment Project Financing *(enriched from source, 2026-07-21)* |

## Description

Extends West Africa Regional Communications Infrastructure Program (WARCIP) with additional financing for broadband backbone in West Africa.

## Source

https://projects.worldbank.org/en/projects-operations/project-detail/P155876 - Project portal, accessed 2026-06-24

## Development history

*(empty at build. Definite-match reporting appends one dated, attributed line here.)*

## Notes

Crosswalk confidence: **high**.

Enrichment: wb-api ok.

Flags: published raised to day precision from enriched board-approval date

The capture labelled **74 of its 75 World Bank rows** `Concessional loan` regardless of the actual instrument. Verified against the World Bank portal, seven are IBRD loans and therefore not concessional; the portal's `idacommamt` field does not separate an IDA credit from an IDA grant, so for IDA operations the label is **unverified** — treat it as "IDA commitment", not established as a loan, unless a primary says otherwise.
