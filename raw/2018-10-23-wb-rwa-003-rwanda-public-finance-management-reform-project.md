---
type: source
title: "Rwanda Public Finance Management Reform Project"
url: https://projects.worldbank.org/en/projects-operations/project-detail/P164807
publisher: World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A)
published: 2018-10-23
date_precision: day
date_source: source
places: [RWA]
topics: [dpi.govtech, finance.new]
entities: [[world-bank], [republic-of-rwanda]]
financier_slug: world-bank
recipient_slug: republic-of-rwanda
lens: []
deal_id: wb-rwa-003
finance_origin: non-state
ingested: 2026-07-21
retrieved: 2026-06-24
---
# Rwanda Public Finance Management Reform Project

World Bank Group committed US$20,000,000 to Republic of Rwanda (RWA).

## Deal record

| Field | Value |
|---|---|
| Deal ID | wb-rwa-003 |
| Financier | World Bank Group — IDA/IBRD (Digital Economy for Africa, DE4A) |
| Recipient | Republic of Rwanda (RWA) |
| Beneficiary type | Government |
| Instrument | Concessional loan *(source label, unverified — see Notes)* |
| Status | Closed |
| Commitment (USD) | US$20,000,000 |
| Original amount | USD 20,000,000 |
| Commitment year | 2018 |
| Start year | 2018 |
| End year | 2024 |
| Amount quality | Exact |
| WB DT category | Core Government Systems -> dpi.govtech |
| Digital ID flag | FALSE |
| Digital payments flag | FALSE |
| Project ID | P164807 |
| IATI activity ID | 44000-P164807 |
| Board approval date | 2018-10-23 *(enriched from source, 2026-07-21)* |
| Closing date | 2024-12-31 *(enriched from source, 2026-07-21)* |
| Borrower | Republic of Rwanda *(enriched from source, 2026-07-21)* |
| Implementing agency | Ministry of Finance and Economic Planning *(enriched from source, 2026-07-21)* |
| Status (WB) | Active *(enriched from source, 2026-07-21)* |
| Lending instrument | Investment Project Financing *(enriched from source, 2026-07-21)* |

## Description

Improves budget reliability and control of funds for service delivery, enhances budget transparency and increases professionalization of public finance officials in Rwanda. Component 1 focuses on PFM IT Systems Roll Out — deployment and stabilization of IFMIS across government, electronic payroll systems, and financial reporting systems.

## Source

https://projects.worldbank.org/en/projects-operations/project-detail/P164807 - Project portal, accessed 2026-06-24

## Development history

*(empty at build. Definite-match reporting appends one dated, attributed line here.)*

## Notes

Re-discovery: Tier 1 keyword "IFMIS" in description. Component 1 = "PFM IT Systems Roll Out" as primary digital deliverable → 140400.

Crosswalk confidence: **high**.

Enrichment: wb-api ok.

Flags: published raised to day precision from enriched board-approval date

The capture labelled **74 of its 75 World Bank rows** `Concessional loan` regardless of the actual instrument. Verified against the World Bank portal, seven are IBRD loans and therefore not concessional; the portal's `idacommamt` field does not separate an IDA credit from an IDA grant, so for IDA operations the label is **unverified** — treat it as "IDA commitment", not established as a loan, unless a primary says otherwise.
