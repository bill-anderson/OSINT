...
<!-- URL note: the URL listed in the prior run's manifest
     (`.../kiroo-games-ceo-payment-agregation-monopoly-threat`) is **truncated and 404s**. The
     live slug ends `...-monopoly-threatens-cameroons-digital-economy` and is recorded above.
     Same for the Anthony Same item — see drop log / manifest. -->

<!-- Byline and date established from the fetched page, which carries an in-body line
     "By : Idriss Linge / Date : jeudi, 27 février 2025 14:48". Agrees with Exa metadata.
     (Note the outlet's own headline misspells "Aggregation" as "Agregation"; the title is
     transcribed verbatim.) -->

<!-- Note for ingest: **the sharpest critical voice in the entire CMR sweep, and the only item
     that shows a Cameroonian digital-policy decision inflicting measurable harm on a
     Cameroonian firm.** Everything else staged for CMR is announcement-shaped; this is an
     on-the-record interview with a named founder describing damage.

     What it reports: the **Ministry of Territorial Administration** granted **InTouch
     exclusive rights** over online-gaming payment aggregation. Because the directive used the
     legal term "online games", it swept in **video-game studios** alongside gambling and
     lottery operators. Kiro'o Games — Central Africa's first professional game studio — was
     given **three weeks** to switch payment providers and reported **over ten days with no
     customer payments at all**, across its whole cross-media catalogue (games, colouring
     books, comics, Rebuntu), not just games. Madiba says that for a studio wholly dependent on
     the Cameroonian market, the decision "could push us straight into bankruptcy".

     Why it matters against held material: the CMR holdings carry the state's sovereignty
     rhetoric extensively (CENADI, Zamengoe, sovereign cloud, SNIA) but almost nothing on
     what state control of payment rails *costs* the local digital economy. This supplies
     it — and does so from a source that explicitly **supports** the underlying
     regulate-gambling-flows objective while attacking the instrument. That is a more useful
     critique than blanket opposition. Madiba also makes the concentration argument
     (~80% of Francophone African online payments run on Mobile Money; monopoly removes the
     redundancy studios rely on) and the investment-climate argument (Central Africa already
     attracts less VC than Kenya or Nigeria).

     Tagged `colonialism` as well as `sovereignty`: Madiba frames Kiro'o's purpose as offering
     "gaming experiences rooted in African cultural references" against "a market largely
     dominated by Western or Asian models" — the lens is his own, not imposed.

     Gaps this surfaces: **InTouch has no entity page** despite now holding a state-granted
     payments monopoly; nor does Kiro'o Games; nor the Ministry of Territorial Administration.
     The underlying directive is not held either — **the primary is worth sourcing at ingest**,
     since no staged source carries its text, date or scope. -->

# Kiro'o Games CEO: Payment Agregation Monopoly Threatens Cameroon's Digital Economy

*By: Idriss Linge — Date: jeudi, 27 février 2025 14:48*
...
We are competing on a global scale, and to succeed, we need to operate as a united and well-structured ecosystem. In many countries where online payment services are thriving, regulation plays a crucial role.
...
**WAT:** [On how a typical Mobile Money digital payment works]

**Olivier Madiba:** Here's how a typical digital payment with Mobile Money works: Once the customer provides this information, Kiro'o's server connects with the payment aggregator's server. The aggregator's server then communicates with the Mobile Money provider's server, either Orange or MTN. The player completes the payment through Orange or MTN. The payment confirmation is sent back to the aggregator, who then forwards it to us. Finally, the player receives payment confirmation and the digital content within the game.
...
*[Body captured at fetch 2026-07-17. The fetched capture truncated mid-answer in the fourth
question; the final portion is completed from the visible page text.]*
